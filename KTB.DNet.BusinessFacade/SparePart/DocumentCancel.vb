...
Namespace KTB.DNET.BusinessFacade.SparePart
    ...
    Public Class DocumentCancelFacade
        Inherits AbstractFacade

#Region "Private Variables"

        Private m_userPrincipal As IPrincipal = Nothing
        Private m_DocumentCancelMapper As IMapper
...
#Region "Constructor"
        ...
        Public Sub New(ByVal userPrincipal As IPrincipal)
            ...
            Me.m_DocumentCancelMapper = MapperFactory.GetInstance.GetMapper(GetType(DocumentCancel).ToString)
            ...
            Me.DomainTypeCollection.Add(GetType(DocumentCancel))
...
#Region "Retrieve"
        ...
        Public Function Retrieve(ByVal DocNumber As String, ByVal DocType As EnumDocumentCancel.DocumentType) As DocumentCancel
            Dim criterias As CriteriaComposite = New CriteriaComposite(New Criteria(GetType(DocumentCancel), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
            criterias.opAnd(New Criteria(GetType(DocumentCancel), "DocNumber", MatchType.Exact, DocNumber))
            criterias.opAnd(New Criteria(GetType(DocumentCancel), "DocType", MatchType.Exact, CType(DocType, Short)))

            Dim SparePartEDocumentColl As ArrayList = New ArrayList
            SparePartEDocumentColl = m_DocumentCancelMapper.RetrieveByCriteria(criterias)
            If Not IsNothing(SparePartEDocumentColl) AndAlso SparePartEDocumentColl.Count > 0 Then
                Return CType(SparePartEDocumentColl(0), DocumentCancel)
            End If
            Return New DocumentCancel
        End Function
        ...
        '                                 Optional ByVal NomorBilling As String = "", Optional ByVal varAmount As Double = 0) As DataSet
...
#Region "Custom Method"
        ...
        Private Sub m_TransactionManager_Insert(ByVal sender As Object, ByVal InsertArg As TransactionManager.OnInsertArgs)
            ...
            If (TypeOf InsertArg.DomainObject Is DocumentCancel) Then
                CType(InsertArg.DomainObject, DocumentCancel).ID = InsertArg.ID
                CType(InsertArg.DomainObject, DocumentCancel).MarkLoaded()
            End If
        End Sub
        ...
        '            Return returnValue
        '        End Function
        'Sub InsertFromWebSevice(objSparePartBilling As SparePartBilling)
        '    Throw New NotImplementedException
        'End Sub
        Public Function InsertFromWebSevice(ByVal spBill As DocumentCancel) As Short
            ...
            If Me.IsTaskFree() Then
                ...
                Dim spBill_DB As DocumentCancel = ValidateDocumentCancel(spBill)
        ...
        Public Function ValidateDocumentCancel(ByVal DocumentCancel As DocumentCancel) As DocumentCancel
            Try
                Dim criteria As CriteriaComposite = New CriteriaComposite(New Criteria(GetType(KTB.DNET.Domain.DocumentCancel), "DocNumber", MatchType.Exact, DocumentCancel.DocNumber))
                criteria.opAnd(New Criteria(GetType(KTB.DNET.Domain.DocumentCancel), "DocType", MatchType.Exact, DocumentCancel.DocType))
                Dim arlDocumentCancel As ArrayList = m_DocumentCancelMapper.RetrieveByCriteria(criteria)
                If arlDocumentCancel.Count > 0 Then
                    Return CType(arlDocumentCancel(0), DocumentCancel)
                Else
                    Return Nothing
                End If
            Catch ex As Exception
