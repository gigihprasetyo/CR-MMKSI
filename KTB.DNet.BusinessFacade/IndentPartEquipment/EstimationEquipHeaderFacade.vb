...
#Region "Retrieve"
        ...
        Public Function RetrieveByDealer(ByVal intDealerId) As ArrayList
            ...
            criterias.opAnd(New Criteria(GetType(EstimationEquipHeader), "Dealer", MatchType.Exact, intDealerId))
...
#Region "Transaction/Other Public Method"
        ...
        Public Function UpdateEstimationEquipHeader(ByVal objDomain As KTB.DNet.Domain.EstimationEquipHeader, ByVal arrIPDetail As ArrayList) As Integer
            ...
            If (Me.IsTaskFree()) Then
                Try
                    Dim oEEHnDB As EstimationEquipHeader = m_EstimationEquipHeaderMapper.Retrieve(objDomain.ID)
                    Me.SetTaskLocking()
                    'm_TransactionManager.AddInsert(objDomain, m_userPrincipal.Identity.Name)
                    If arrIPDetail.Count > 0 Then
                        For Each objIPDetail As EstimationEquipDetail In arrIPDetail
                    ...
                    If objDomain.ID > 0 Then
                        Me.WriteHistory(oEEHnDB, objDomain)
                    End If
                    returnValue = objDomain.ID
                Catch ex As Exception
                    Dim rethrow As Boolean = ExceptionPolicy.HandleException(ex, "Domain Policy")
                ...
                Try
                    ...
                    If arlIPH.Count > 0 Then
                        For Each objIPHH As EstimationEquipHeader In arlIPH
                            Dim oEEHInDB As EstimationEquipHeader = m_EstimationEquipHeaderMapper.Retrieve(objIPHH.ID)
                            m_TransactionManager.AddUpdate(objIPHH, m_userPrincipal.Identity.Name)
                            Me.WriteHistory(objIPHH, oEEHInDB)
                        Next
                    End If
                    m_TransactionManager.PerformTransaction()
                    returnValue = 1
        ...
        Public Function Update(ByVal objDomain As EstimationEquipHeader) As Integer
            ...
            Try
                Dim oEEHnDB As EstimationEquipHeader = m_EstimationEquipHeaderMapper.Retrieve(objDomain.ID)
                Dim ID As Integer = m_EstimationEquipHeaderMapper.Update(objDomain, m_userPrincipal.Identity.Name)

                If ID > 0 Then
                    Me.WriteHistory(objDomain, oEEHnDB)
                End If
                Return ID
            Catch ex As Exception
                Dim rethrow As Boolean = ExceptionPolicy.HandleException(ex, "Domain Policy")
...
#Region "Custom Method"
        ...
        Public Sub RecordStatusChangeHistory(ByVal item As EstimationEquipHeader, ByVal oldStatus As Integer)
            ...
            If Not item Is Nothing Then
                ...
                objStatusChangeHistoryFacade.Insert(CInt(LookUp.DocumentType.Estimation_Equip), item.EstimationNumber, oldStatus, item.Status)
            End If
        End Sub
...
#Region "DataHistory"
        ...
        Public Function WriteHistory(ByVal oCR As EstimationEquipHeader, ByVal oCRinDB As EstimationEquipHeader) As Integer
            ...
            If oCR.ID > 0 Then
                ...
                'Dim oCRinDB As EstimationEquipHeader = Me.m_EstimationEquipHeaderMapper.Retrieve(oCR.ID)
                Dim oDH As New DataHistory
                Dim oDHD As DataHistoryDetail
                Dim aDHDs As New ArrayList
                Dim IsChanged As Boolean = False

                oDH.DataTable = "EstimationEquipHeader"
                oDH.DataID = oCR.ID
                ...
                If IsChanged Then
                    ...
                    cDH.opAnd(New Criteria(GetType(DataHistory), "DataTable", MatchType.Exact, "EstimationEquipHeader"))
