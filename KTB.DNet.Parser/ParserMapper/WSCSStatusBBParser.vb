...
Namespace KTB.DNet.Parser
    ...
    Public Class WSCStatusBBParser
        Inherits AbstractParser

#Region "Private Variables"
        Private stream As StreamReader
        Private grammar As Regex
        Private WSCHeaderBBs As ArrayList
        Private WSCDetailBBs As ArrayList
        Private _fileName As String
        Private _WSCHeaderBB As WSCHeaderBB 'Header
        Private _WSCDetailBB As WSCDetailBB
...
#Region "Protected Methods"
        ...
        Protected Overrides Function DoParse(ByVal fileName As String, ByVal user As String) As Object
            Try
                _fileName = fileName
                Dim val As String
                WSCHeaderBBs = New ArrayList
                stream = New StreamReader(fileName, True)
                val = MyBase.NextLine(stream).Trim()
                While (Not val = "")
                    Try
                        If val.Substring(0, 1).Equals("H") Then
                            If Not _WSCHeaderBB Is Nothing Then
                                WSCHeaderBBs.Add(_WSCHeaderBB)  '-- WSC header input text
                            End If
                            _WSCHeaderBB = ParseWSCHeaderBB(val + delimited)
                        Else
                            If Not _WSCHeaderBB Is Nothing Then
                                ParseWSCDetailBB(val + delimited, _WSCHeaderBB)  'Order detail input
                            End If
                        End If
                    Catch ex As Exception
                        ...
                        _WSCHeaderBB = Nothing
                    End Try
                    val = MyBase.NextLine(stream)
                End While

                If Not _WSCHeaderBB Is Nothing Then
                    WSCHeaderBBs.Add(_WSCHeaderBB)
                End If
            Catch ex As Exception
                Throw ex
            Finally
        ...
        Protected Overrides Function DoTransaction() As Integer
            For Each item As WSCHeaderBB In WSCHeaderBBs
                Try
                    Dim objHeaderFromDB As WSCHeaderBB = GetWSC(item)
        ...
        Private Sub InsertWsc(ByVal obj As WSCHeaderBB)
            Dim _headerFacade As New WSCHeaderBBFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WSM"), Nothing))
            _headerFacade.Insert(obj)
        End Sub

        Private Sub UpdateWsc(ByVal objOld As WSCHeaderBB, ByVal objNew As WSCHeaderBB)
            Dim _headerFacade As New WSCHeaderBBFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WSM"), Nothing))
...
#Region "Private Methods"
        ...
        Private Function ParseWSCHeaderBB(ByVal ValParser As String) As WSCHeaderBB
            _WSCHeaderBB = New WSCHeaderBB
                ...
                Select Case nCount
                    ...
                    Case Is = 2  '-- Claim #
                        If sTemp.Length = 6 Then 'Harus 6 digit
                            _WSCHeaderBB.ClaimNumber = sTemp
                        Else
                            ErrorMessage.Append("Claim Number tidak valid." & Chr(13) & Chr(10))
                        End If
                    Case Is = 3  '-- Claim type
                        If sTemp.Length > 0 Then
                            _WSCHeaderBB.ClaimType = sTemp
                        Else
                            ErrorMessage.Append("Claim Type tidak valid." & Chr(13) & Chr(10))
                        End If
                    Case Is = 4  '-- Ref. claim #
                        _WSCHeaderBB.RefClaimNumber = sTemp
                    Case Is = 5  '-- Chassis #
                        If sTemp.Length > 0 Then
                            Dim _chassis As ChassisMasterBB = New ChassisMasterBBFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WSM"), Nothing)).Retrieve(sTemp)
                            If _chassis.ID > 0 Then
                                _WSCHeaderBB.ChassisMasterBB = _chassis
                            Else
                    ...
                    Case Is = 6  '-- Claim date (Service date)
                        Try
                            _WSCHeaderBB.ServiceDate = New Date(sTemp.Substring(4, 4), sTemp.Substring(2, 2), sTemp.Substring(0, 2))
                        Catch ex As Exception
                    ...
                    Case Is = 7  '-- Mileage
                        If sTemp.Length > 0 Then
                            Try
                                _WSCHeaderBB.Miliage = CType(sTemp, Integer)
                            Catch ex As Exception
                        ...
                        End If
                    Case Is = 8  '-- PQR
                        _WSCHeaderBB.PQR = sTemp
                    Case Is = 9  '-- PQR Status
                        _WSCHeaderBB.PQRStatus = sTemp
                    Case Is = 10  '-- Kode A
                        _WSCHeaderBB.CodeA = sTemp
                    Case Is = 11  '-- Kode B
                        _WSCHeaderBB.CodeB = sTemp
                    Case Is = 12  '-- Kode C
                        _WSCHeaderBB.CodeC = sTemp
                    Case Is = 13  '-- Description
                        _WSCHeaderBB.Description = sTemp
                    Case Is = 14  '-- Photo
                        _WSCHeaderBB.EvidencePhoto = sTemp
                    Case Is = 15  '-- Kwitansi
                        _WSCHeaderBB.EvidenceInvoice = sTemp
                    Case Is = 16  '-- Damage
                        _WSCHeaderBB.EvidenceDmgPart = sTemp
                    Case Is = 17  '-- Notification #
                        _WSCHeaderBB.NotificationNumber = sTemp
                    Case Is = 18  '-- Claim status (Update DNet if any)
                        If sTemp.Length > 0 Then
                            If sTemp = "WFAP" Then
                                _WSCHeaderBB.Status = enumStatusWSC.Status.Proses  '-- WSC status = Selesai
                                _WSCHeaderBB.ClaimStatus = sTemp
                            Else
                                _WSCHeaderBB.Status = enumStatusWSC.Status.Selesai  '-- WSC status = Selesai
                                _WSCHeaderBB.ClaimStatus = sTemp
                            End If
                        Else
                        ...
                        If sTemp.Length > 0 Then
                            Try
                                ...
                                If _reason.ID > 0 Then
                                    _WSCHeaderBB.Reason = _reason
                                Else
                                    _WSCHeaderBB.Reason = Nothing
                                End If
                            Catch
                                _WSCHeaderBB.Reason = Nothing
                    ...
                    Case Is = 20  '-- Decide date
                        If sTemp.Length > 0 Then
                            Try
                                _WSCHeaderBB.DecideDate = New Date(sTemp.Substring(4, 4), sTemp.Substring(2, 2), sTemp.Substring(0, 2))
                            Catch ex As Exception
                                ErrorMessage.Append("Decide date tidak valid." & Chr(13) & Chr(10))
                            End Try
                        Else
                            _WSCHeaderBB.DecideDate = New Date(1900, 1, 1) ' if date is blank replace with minimum date
                        End If
                    Case Is = 21  '-- Labor amount (Update DNet if any)
                        If sTemp.Length > 0 Then
                            Try
                                _WSCHeaderBB.LaborAmount = CType(sTemp, Double)
                            Catch ex As Exception
                                ErrorMessage.Append("Labor amount tidak valid." & Chr(13) & Chr(10))
                            End Try
                        Else
                            _WSCHeaderBB.LaborAmount = 0
                        End If
                    Case Is = 22  '-- Part amount (Update DNet if any)
                        If sTemp.Length > 0 Then
                            Try
                                _WSCHeaderBB.PartAmount = CType(sTemp, Double)
                            Catch ex As Exception
                                ErrorMessage.Append("Part amount tidak valid." & Chr(13) & Chr(10))
                            End Try
                        Else
                            _WSCHeaderBB.PartAmount = 0
                        End If
                        'Case Is = 23  '-- Reason  2
                        '    If sTemp.Length > 0 Then
                        '        Try
                        ...
                        '                _WSCHeaderBB.Reason2 = _reason2
                        '            Else
                        '                _WSCHeaderBB.Reason2 = Nothing
                        '            End If
                        '        Catch
                        '            _WSCHeaderBB.Reason2 = Nothing
                        '            ErrorMessage.Append("Reason Id tidak valid." & Chr(13) & Chr(10))
                        ...
                        '                _WSCHeaderBB.Reason3 = _reason3
                        '            Else
                        '                _WSCHeaderBB.Reason3 = Nothing
                        '            End If
                        '        Catch
                        '            _WSCHeaderBB.Reason3 = Nothing
                        '            ErrorMessage.Append("Reason Id tidak valid." & Chr(13) & Chr(10))
                        ...
                        '                _WSCHeaderBB.Reason4 = _reason4
                        '            Else
                        '                _WSCHeaderBB.Reason4 = Nothing
                        '            End If
                        '        Catch
                        '            _WSCHeaderBB.Reason4 = Nothing
                        '            ErrorMessage.Append("Reason Id tidak valid." & Chr(13) & Chr(10))
                        ...
                        '                _WSCHeaderBB.Reason5 = _reason5
                        '            Else
                        '                _WSCHeaderBB.Reason5 = Nothing
                        '            End If
                        '        Catch
                        '            _WSCHeaderBB.Reason5 = Nothing
                        '            ErrorMessage.Append("Reason Id tidak valid." & Chr(13) & Chr(10))
        ...
        Private Sub ParseWSCDetailBB(ByVal ValParser As String, ByVal _objWSCHeaderBB As WSCHeaderBB)
            _WSCDetailBB = New WSCDetailBB
            Dim sStart As Integer
            Dim nCount As Integer
            Dim sTemp As String
            ...
            For Each m As Match In grammar.Matches(ValParser)
                ...
                Select Case nCount
                                ...
                                Case Is = "P"  '-- Sparepart
                                    Try
                                        ...
                                        If _sppart.ID > 0 Then
                                            _WSCDetailBB.SparePartMaster = _sppart
                                        Else
                    ...
                    Case Is = 5  '-- If Type=L then "WorkCode" (ignore) else if Type=P then "Quantity"
                        Select Case _WSCDetailBB.WSCType
                            Case Is = "L"  '-- Labor
                                Position = sTemp
                                If Position <> "99" Then
                                    Dim _laborMaster As LaborMaster = GetLaborMaster(TempLabor, sTemp, _objWSCHeaderBB.ChassisMasterBB)
                                    If Not _laborMaster Is Nothing Then
                                        _WSCDetailBB.LaborMaster = _laborMaster
                                    Else
                                        ErrorMessage.Append("Labor code tidak terdefinisi" & Chr(13) & Chr(10))
                                    End If
                                End If
                            ...
                            Case Is = "P"  '-- Sparepart
                                If sTemp.Length > 0 Then
                                    Try
                                        _WSCDetailBB.Quantity = CType(sTemp, Double)
                                    Catch
                    ...
                    Case Is = 6  '-- If Type=L then "Quantity" else if Type=P then "PartPrice"
                        Select Case _WSCDetailBB.WSCType
                            Case Is = "L"  '-- Labor
                                If Position = "99" Then
                                    Try
                                        _WSCDetailBB.PartPrice = CType(sTemp, Double)
                                    Catch ex As Exception
                                        _WSCDetailBB.PartPrice = 0
                                    End Try
                                Else
                                    Try
                                        _WSCDetailBB.Quantity = CType(sTemp, Double)
                                    Catch ex As Exception
                                        _WSCDetailBB.Quantity = 0
                                    End Try
                                End If
                            Case Is = "P"  '-- Sparepart
                                If sTemp.Length > 0 Then
                    ...
                    Case Is = 7  '-- If Type=P then "MainPart" else if Type=P then "0"
                        Select Case _WSCDetailBB.WSCType
                            Case Is = "L"  '-- Labor
                                _WSCDetailBB.MainPart = 0
                            Case Is = "P"  '-- Sparepart
                                If sTemp = "X" Then
                                    _WSCDetailBB.MainPart = 1
                                Else
                                    _WSCDetailBB.MainPart = 0
                                End If
                        End Select
            ...
            If ErrorMessage.Length > 0 Then
                Throw New Exception(ErrorMessage.ToString)
            End If
            _objWSCHeaderBB.wSCDetailBBs.Add(_WSCDetailBB)
        End Sub

        Private Function GetLaborMaster(ByVal LaborCode As String, ByVal position As String, ByVal objChassisMasterBB As ChassisMasterBB) As LaborMaster
            Dim vecTypeID As Integer = objChassisMasterBB.VechileColor.VechileType.ID
        ...
        Private Function isExistsWSC(ByVal _header As WSCHeaderBB) As Boolean
            '-- Check to see if _header exists in DNet based on Dealer ID and Claim number

            Dim criterias As New CriteriaComposite(New Criteria(GetType(WSCHeaderBB), "Dealer.ID", MatchType.Exact, _header.Dealer.ID))
            criterias.opAnd(New Criteria(GetType(WSCHeaderBB), "ClaimNumber", MatchType.Exact, _header.ClaimNumber))
            ...
            If headerColl.Count > 0 Then
                ...
                _header.ID = CType(headerColl(0), WSCHeaderBB).ID
            End If
            Return headerColl.Count > 0

        End Function

        Private Function GetWSC(ByVal _header As WSCHeaderBB) As WSCHeaderBB
            Dim criterias As New CriteriaComposite(New Criteria(GetType(WSCHeaderBB), "Dealer.ID", MatchType.Exact, _header.Dealer.ID))
            criterias.opAnd(New Criteria(GetType(WSCHeaderBB), "ClaimNumber", MatchType.Exact, _header.ClaimNumber))
            Dim _headerFacade As New WSCHeaderBBFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WSM"), Nothing))
            Dim headerColl As ArrayList = _headerFacade.Retrieve(criterias)
            If headerColl.Count > 0 Then
                Return CType(headerColl(0), WSCHeaderBB)
            Else
                Return Nothing
            End If
        End Function
