...
Namespace KTB.DNet.Parser
    Public Class SPPendingOrderBOParser
...
#Region "Protected Methods"
        ...
        Protected Overrides Function DoParse(ByVal fileName As String, ByVal user As String) As Object
            ...
            Catch ex As Exception
                SysLogParameter.LogErrorToSyslog(ex.Message.ToString, "wsm-worker", "SPPendingOrderBOParser.vb", "Parsing", KTB.DNet.[Lib].DNetLogFormatStatus.Direct, SourceName, SysLogParameter.ParserType.SPPendingOrderBOParser, BlockName)
            End Try
        ...
        Protected Overrides Function DoTransaction() As Integer
            If _PendingOrders.Count > 0 Then
                Dim objSPPo As SparePartPendingOrder
                For Each spPOHeader As SparePartPendingOrder In _PendingOrders
                    Try
                        ...
                        objSPPo = New SparePartPendingOrder
                        objSPPo = New SparePartPendingOrderFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WSM"), Nothing)).ValidateSPPO(spPOHeader)

                        If Not IsNothing(objSPPo) Then
                            spPOHeader.ID = objSPPo.ID
                            Dim nResult = New SparePartPendingOrderFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WSM"), Nothing)).Update(spPOHeader, objSPPo)
                        Else
                            Dim nResult = New SparePartPendingOrderFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WSM"), Nothing)).Insert(spPOHeader)
                        End If

                    Catch ex As Exception
                        SysLogParameter.LogErrorToSyslog(ex.Message.ToString, "wsm-worker", "SPPendingOrderBOParser.vb", "Transaction", KTB.DNet.[Lib].DNetLogFormatStatus.Direct, SourceName, SysLogParameter.ParserType.SPPendingOrderBOParser, BlockName)
...
#Region "Private Methods"
        ...
        Private Function ParseSPPOHeader(ByVal val As String) As SparePartPendingOrder
            ...
            If Not IsNothing(sPPO) Then
                ...
                objSPPOPendingOrder.DocumentType = "B" 'Document type B=Back order; N=Normal
