...
Namespace KTB.DNet.Parser
    ...
    Public Class TOPSPTransferCeilingUpdateParser
...
#Region "Private Variables"
        ...
        Private _XTC As TOPSPTransferCeiling
        Private _XTCD As TOPSPTransferCeilingDetail
...
#Region "Protected Methods"
        ...
        Protected Overrides Function DoTransaction() As Integer
            ...
            Dim TCDFac As New TOPSPTransferCeilingDetailFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WS"), Nothing))
            ...
            If IsNothing(_TCH.ErrorMessage) OrElse _TCH.ErrorMessage.ToString() = String.Empty Then
                Dim StrSQLHeader As String = " exec TOPSPTransferCeiling_BalanceUpdate @ProductCategoryID={0}, @CreditAccount='{1}', @PaymentType={2}, @EffectiveDate='{3}', @BalanceBefore={4}"
                ...
                Try
                    ...
                    Else
                        ...
                        If oTC.IsIncome = 1 Then
                            ...
                            StrSQLDetail = String.Format(StrSQLDetail, oTC.IsIncome, oTC.TransDate.ToString("yyyy/MM/dd"), oTC.TOPSPTransferPayment.ID)
                        Else
                            StrSQLDetail = StrSQLDetail & ", @IsIncome={0},@TransDateDetail='{1}', @SparePartBillingID={2} "
                            StrSQLDetail = String.Format(StrSQLDetail, oTC.IsIncome, oTC.TransDate.ToString("yyyy/MM/dd"), oTC.SparePartBilling.ID)
                        End If
        ...
        Private Function ParseHeader(ByVal line As String) As TCHeader
            ...
            Else
                ...
                Else
                    ...
                    ElseIf cols(2).Trim = "S102" Then
                        _ObjTch.ProductCategoryID = 1
                    ElseIf cols(2).Trim = "S200" Then
                        _ObjTch.ProductCategoryID = 1
                    End If
                End If
                ...
                Else
                    ...
                    cTC.opAnd(New Criteria(GetType(TOPSPTransferCeiling), "ProductCategory.ID", MatchType.Exact, _ObjTch.ProductCategoryID))
                    cTC.opAnd(New Criteria(GetType(TOPSPTransferCeiling), "CreditAccount", MatchType.Exact, _ObjTch.CreditAccount))
                    cTC.opAnd(New Criteria(GetType(TOPSPTransferCeiling), "PaymentType", MatchType.Exact, _ObjTch.PaymentType))
                    cTC.opAnd(New Criteria(GetType(TOPSPTransferCeiling), "EffectiveDate", MatchType.Exact, DateTime.Now.ToString("yyyy.MM.dd")))
        ...
        Private Function ParseDetail(ByVal line As String) As TCDetail
            ...
            Else
                ...
                If cols(1).Trim().ToLower() = "b" Then
                    If cols(2).Trim = String.Empty Then
                        writeError("SP Billing Number can't be empty")
                    Else
                        oSO = oSOFac.Retrieve(cols(2).Trim())
                    End If
                    If oSO Is Nothing OrElse oSO.ID < 1 Then
                        writeError("Invalid SP Billing Number")
                    Else
...
#Region "Private Class"
        Private Class TCDetail
            Inherits TOPSPTransferCeilingDetail
