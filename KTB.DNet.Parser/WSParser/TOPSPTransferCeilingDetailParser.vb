...
Namespace KTB.DNet.Parser
    ...
    Public Class TOPSPTransferCeilingDetailParser
...
#Region "Private Variables"
        ...
        Private _TCD As TOPSPTransferCeilingDetail
...
#Region "Protected Methods"
                        ...
                        ElseIf ind = MyBase.IndicatorDetail Then
                            ...
                            Else
                                ...
                                If Not IsNothing(_TCD) AndAlso (IsNothing(_TCD.ErrorMessage) OrElse _TCD.ErrorMessage = String.Empty) Then
                                    _TCD.TOPSPTransferCeiling = _TC
                                    _TC.TOPSPTransferCeilingDetails.Add(_TCD)
                                End If
                            End If
                        End If
        ...
        Protected Overrides Function DoTransaction() As Integer
            Dim TCFac As New TOPSPTransferCeilingFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WS"), Nothing))
            Dim TCDFac As TOPSPTransferCeilingDetailFacade
            Dim nError As Integer = 0
            Dim sMsg As String = ""

            For Each oTC As TOPSPTransferCeiling In _TCs
                Try
                ...
                Catch ex As Exception
                    ...
                    SysLogParameter.LogErrorToSyslog(ex.Message.ToString(), "ws-worker", "TOPSPTransferCeilingDetailParser.vb", "Transaction", KTB.DNet.[Lib].DNetLogFormatStatus.Direct, SourceName, WSMSyslogParameter.ParserType.PurchaseOrderParser, BlockName)
            ...
            If nError > 0 Then
                SysLogParameter.LogErrorToSyslog("Error Message :" & sMsg, "ws-worker", "TOPSPTransferCeilingDetailParser.vb", "Transaction", KTB.DNet.[Lib].DNetLogFormatStatus.Direct, SourceName, WSMSyslogParameter.ParserType.PurchaseOrderParser, BlockName)
...
#Region "Private Methods"
        ...
        Private Function ParseHeader(ByVal line As String) As TOPSPTransferCeiling
            ...
            Else
                ...
                If Not IsNothing(errorMessage) AndAlso errorMessage.ToString().Trim() <> "" Then
                    If IsNothing(_TC) Then _TC = New TOPSPTransferCeiling
                    _TC.ErrorMessage = errorMessage.ToString()
                Else
                    _TC.CreatedBy = "SAP WS"

                    Dim aTCs As ArrayList
                    Dim TCFac As New TOPSPTransferCeilingFacade(user)
                    ...
                    Dim oTCDb As TOPSPTransferCeiling

                    cTC.opAnd(New Criteria(GetType(TOPSPTransferCeiling), "ProductCategory.ID", MatchType.Exact, _TC.ProductCategory.ID))
                    cTC.opAnd(New Criteria(GetType(TOPSPTransferCeiling), "CreditAccount", MatchType.Exact, _TC.CreditAccount))
                    cTC.opAnd(New Criteria(GetType(TOPSPTransferCeiling), "PaymentType", MatchType.Exact, _TC.PaymentType))
                    cTC.opAnd(New Criteria(GetType(TOPSPTransferCeiling), "EffectiveDate", MatchType.Exact, _TC.EffectiveDate.ToString("yyyy.MM.dd")))
        ...
        Private Function ParseDetail(ByVal line As String) As TOPSPTransferCeilingDetail
            Dim cols As String() = line.Split(MyBase.ColSeparator)
            Dim user As GenericPrincipal = New System.Security.Principal.GenericPrincipal(New GenericIdentity("ws"), Nothing)
            Dim oSOFac As New SparePartBillingFacade(user)
            Dim oSO As New SparePartBilling
            Dim oTPFac As New TOPSPTransferPaymentFacade(user)
            Dim oTP As New TOPSPTransferPayment

            _TCD = New TOPSPTransferCeilingDetail()
            errorMessage = New StringBuilder()
            ...
            Else
                ...
                '2:              Num()
                '3:              Amont()
                If cols(1).Trim().ToLower() = "b" Then
                    If cols(2).Trim = String.Empty Then
                        writeError("Biilling Number can't be empty")
                    Else
                        oSO = oSOFac.Retrieve(cols(2).Trim())
                    End If
                    If oSO Is Nothing OrElse oSO.ID < 1 Then
                        writeError("Invalid Biilling Number")
                    Else
                        _TCD.SparePartBilling = oSO 'ID = oSO.ID
                    End If
                    _TCD.IsIncome = 0
                ElseIf cols(1).Trim().ToLower() = "p" Then
                    ...
                    Else
                        _TCD.TOPSPTransferPayment = oTP ' ID = oTP.ID
                    End If
                    _TCD.IsIncome = 1
                End If
                Try
