...
Namespace KTB.DNet.Parser
    ...
    Public Class UploadCustomerCallCenter
        Inherits AbstractExcelParser

#Region "Private Variables"
        Private CcContactList As ArrayList
        Private _ccContact As CcContact
        ...
        Private appConf As AppConfigFacade = New AppConfigFacade(New System.Security.Principal.GenericPrincipal(New GenericIdentity("WSM"), Nothing))
...
#Region "Protected Methods"
        ...
        Sub New(ByVal oStrPeriod As String, ByVal oPeriodID As Integer, ByVal oUserDealer As Dealer, Optional ByVal contentFileType As String = "")
            Me.StrPeriod = oStrPeriod
            Me.periodID = oPeriodID
            Me.UserDealer = oUserDealer
            Me.ContentFileType = contentFileType
            InitCCVehicleOdometer()
        ...
        Protected Overrides Function ParsingExcelNoTransaction(ByVal fileName As String, ByVal sheetName As String, ByVal user As String) As Object
            ...
            Try
                ...
                Else
                    ...
                    'Max Row = 150
                    Dim maxRow As Integer = CInt(appConf.Retrieve("CC.Upload.MaxRow").Value)
                    If Ds.Tables(0).Rows.Count > maxRow Then
                        Dim dr As DataRow
                        Dim iR As Integer = 0
                        For iR = 0 To Ds.Tables(0).Rows.Count - 1
                            If iR > maxRow - 1 Then
                                dr = Ds.Tables(0).Rows(iR)
                                If Not IsDBNull(dr(1)) Or Not IsDBNull(dr(3)) Then
                                    Me.ErrorMessage.Append(String.Format("Jumlah baris lebih dari {0}. ", maxRow))
                                    Exit For
                                End If
                            End If
                        Next
...
#End Region
        ...
        Private Function ParseCcContact(ByVal row As DataRow) As CcContact
            ...
            If Not IsDBNull(row(2)) Then
                Try
                    If CStr(row(2)).Trim.Length > 0 Then
                        _ccContact.Sex = IIf(Left(row(2), 1) = "B", "L", "P")
                    End If
                Catch ex As Exception
            ...
            If Not IsDBNull(row(3)) Then
                Try
                    _ccContact.ConsumerName = row(3)
                Catch ex As Exception
                    'ErrorMessage.Append("Nama Lengkap Konsumen harus diisi." & Chr(13) & Chr(10))
                End Try
            Else
                ErrorMessage.Append("Nama Lengkap Konsumen harus diisi." & Chr(13) & Chr(10))
            End If
            If Not IsDBNull(row(4)) Then
                Try
                    _ccContact.HandphoneNo = row(4)
                    Dim strMessage As String = IsPhoneValid(row(4), "Handphone")
            ...
            If Not IsDBNull(row(5)) Then
                Try
                    _ccContact.HomePhoneAreaCode = row(5)
                Catch ex As Exception
                    'ErrorMessage.Append("" & Chr(13) & Chr(10))
                End Try
            Else
            ...
            If Not IsDBNull(row(6)) Then
                Try
                    _ccContact.HomePhoneNo = row(6)
                    Dim strMessage As String = IsPhoneValid(row(6))
                    If strMessage <> String.Empty Then
                        ErrorMessage.Append(strMessage & Chr(13) & Chr(10))
                    End If
                Catch ex As Exception
            ...
            Dim isCheckSIU As Integer = CInt(appConf.Retrieve("CC.Upload.IsCheckSIU").Value)
            If Not IsDBNull(row(15)) Then
                Try
                    If isCheckSIU = 1 Then
                        ...
                        criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.AssistServiceIncoming), "TglTutupTransaksi", MatchType.GreaterOrEqual, strStart))
                        criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.AssistServiceIncoming), "TglTutupTransaksi", MatchType.Lesser, strEnd))
            ...
            If Not IsDBNull(row(17)) Then
                Try
                    ...
                    If _ccContact.VehicleType <> "" Then
                        ...
                        Dim maxKm As Integer = CInt(appConf.Retrieve("CC.Upload.MaxKM").Value)
                        Dim ODOdometer As Integer = IIf(IsNothing(oDefaultOdometer) OrElse CInt(oDefaultOdometer) <= 0, maxKm, CInt(oDefaultOdometer))
            ...
            If ErrorMessage.Length > 0 Then
                _ccContact.ErrorMessage = ErrorMessage.ToString
