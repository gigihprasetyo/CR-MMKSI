    ...
    Protected Overrides Function executeJob() As Boolean
        ...
        If 1 = 1 Then
            Try
                ...
                sTo = KTB.DNet.Lib.WebConfig.GetValue("IndentPartEquipmentPO1To")
                sCC = KTB.DNet.Lib.WebConfig.GetValue("IndentPartEquipmentPO1CC")
                Dim Dir As String = Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly().Location)

                Dir = Dir + "\EmailTemplate\IndentPartEquipEstimationKTB.htm"

                'Me.SendEmail(Dir, sTo, sCC, "[Indent Part Equipment] Daftar Estimasi Yang Sudah 14 Hari Dan Belum Dikonfirmasi" & Library.GetIPv4Address(), sContents)

                Me.SendEmail(Dir, sTo, sCC, "[MMKSI-DNet] Parts - Daftar Estimasi Indent Part Equipment Yang Sudah 14 Hari Dan Belum Dikonfirmasi", sContents)

                LogHelper.WriteLog("Send Email IndentPartEquipmentPO (1) Success")
            Catch ex As Exception
                LogHelper.WriteLog("error 1 : " & ex.Message & " Line : " & ex.Source & Environment.NewLine & ex.StackTrace)
            End Try
        ...
        If 2 = 2 Then
            Try
                ...
                For Each oD As Dealer In aDs
                    ...
                    sTo &= ";" & KTB.DNet.Lib.WebConfig.GetValue("IndentPartEquipmentPO2To")
                    sCC &= ";" & KTB.DNet.Lib.WebConfig.GetValue("IndentPartEquipmentPO2CC")

                    Dim Dir As String = Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly().Location)

                    Dir = Dir + "\EmailTemplate\IndentPartEquipEstimationDealer.htm"

                    'Me.SendEmail(Dir, sTo, sCC, "[Indent Part Equipment] (" & oD.DealerCode & ") Daftar Order Yang Belum Melakukan Proses Kirim" & Library.GetIPv4Address(), sContents)
                    Me.SendEmail(Dir, sTo, sCC, "[Indent Part Equipment] (" & oD.DealerCode & ") Daftar Order Yang Belum Melakukan Proses Kirim", sContents)
                Next
                LogHelper.WriteLog("Send Email IndentPartEquipmentPO (2) Success")
            Catch ex As Exception
                LogHelper.WriteLog("error 2 : " & ex.Message & " Line : " & ex.Source & Environment.NewLine & ex.StackTrace)
            End Try
        End If
            ...
            Try
                ...
                For Each oD As Dealer In aDs
                    ...
                    If aVEPs.Count > 0 Then
                        ...
                        sTo &= ";" & KTB.DNet.Lib.WebConfig.GetValue("IndentPartEquipmentPO3To")
                        sCC &= ";" & KTB.DNet.Lib.WebConfig.GetValue("IndentPartEquipmentPO3CC")
                        ...
                        Dir = Dir + "\EmailTemplate\IndentPartEquipOrderNoDepositB.htm"

                        'Me.SendEmail(Dir, sTo, sCC, "[Indent Part Equipment] (" & oD.DealerCode & ") Daftar Order Yang Belum Mengirimkan Kwitansi Asli Deposit B" & Library.GetIPv4Address(), sContents)
                        Me.SendEmail(Dir, sTo, sCC, "[MMKSI-DNet] Parts - (" & oD.DealerCode & ") Daftar Order Indent Part Equipment Yang Belum Mengirimkan Kwitansi Asli Deposit B", sContents)
                    End If
                Next
                LogHelper.WriteLog("Send Email IndentPartEquipmentPO (3) Success")
            Catch ex As Exception
                LogHelper.WriteLog("error 3 : " & ex.Message & " Line : " & ex.Source & Environment.NewLine & ex.StackTrace)
            End Try
        End If
    End Function
