...
Public MustInherit Class Job
    ...
    Public Sub DoJob()
        Dim n As Integer = DateDiff(DateInterval.Second, Me.ScheduleDate, Me.CurrentTime)
        'LogHelper.WriteLog("DoJob - schedule checking.(" & n.ToString() & " seconds) " & Me.ScheduleDate.ToString("yyyy.MM.dd HH:mm:ss") & " vs " & Me.CurrentTime.ToString("yyyy.MM.dd HH:mm:ss"))
    ...
    Protected ReadOnly Property EmailTo() As String
        Get
            Return ConfigurationSettings.AppSettings(ModuleName & "To")
        End Get
    End Property

    Protected ReadOnly Property EmailCC() As String
        Get
            Return ConfigurationSettings.AppSettings(ModuleName & "CC")
        End Get
    End Property

    Protected ReadOnly Property Frequency() As Library.Frequency
        Get
            Dim Freq As Library.Frequency = CType(ConfigurationSettings.AppSettings(ModuleName & "Frequency"), Library.Frequency)
    ...
    Protected ReadOnly Property NumOfFrequency() As Integer
        Get
            ...
            Try
                nFreq = CType(ConfigurationSettings.AppSettings(ModuleName & "Custom"), Integer)
    ...
    Protected ReadOnly Property ScheduleDate() As DateTime
        Get
            ...
            Dim sDate As String = ConfigurationSettings.AppSettings(ModuleName & "Date")
            Dim sTime As String = ConfigurationSettings.AppSettings(ModuleName & "Time")
            Dim nFreq As Integer = 0
            Dim dt As DateTime
            Dim dtNext As DateTime

            If Not IsNothing(ConfigurationSettings.AppSettings(ModuleName & "Custom")) Then
                Try
                    nFreq = CType(ConfigurationSettings.AppSettings(ModuleName & "Custom"), Integer)
                Catch ex As Exception
                    nFreq = 0
    ...
    Protected Sub SendEmail(ByVal EmailFile As String, ByVal sTo As String, ByVal sCC As String, ByVal sSubject As String, ByVal sMessage() As String)
        ...
        Dim emailAdmin As String = System.Configuration.ConfigurationSettings.AppSettings("EmailAdmin")
