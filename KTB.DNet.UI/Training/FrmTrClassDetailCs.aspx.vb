...
Public Class FrmTrClassDetailCs
    ...
    Public Sub visibleClassLbl()
        ...
        ICConfirmDueDate.Visible = False
    ...
    Public Sub viewEditClass()
        If Not IsNothing(objClass) Then
            ...
            txtUrlPath.Text = objClass.UrlPath
            ...
            ICConfirmDueDate.Value = objClass.ConfirmDueDate.ToString("dd/MM/yyyy").Trim()
    ...
    Private Sub updateTrClass(ByRef nResult As Integer)
        ...
        With objClass
            ...
            .ConfirmDueDate = ICConfirmDueDate.Value.ToString().Trim()
    ...
    Private Sub insertTrClass(ByVal objTrClass As TrClass, ByRef nResult As Integer)
        ...
        ElseIf ICTanggalSelesai.Value < Today Then
            ...
            Return
        ElseIf ICConfirmDueDate.Value < Today Then
            MessageBox.Show("Tanggal konfirmasi tidak boleh lebih kecil dari tanggal hari ini")
            Return
        End If
        With objTrClass
            .ClassCode = txtKodeKelas.Text.Trim()
...
#End Region
    ...
    Protected Sub cvTanggalMulai_ServerValidate(source As Object, args As ServerValidateEventArgs)
        Try
            Dim tahunFiskal As Integer = CInt(Left(ddlTahunFiscal.SelectedValue.ToString(), 4))
            Dim minDateFiskal As Date = New Date(tahunFiskal, 4, 1)
            Dim maxDateFiskal As Date = New Date(tahunFiskal + 1, 3, 31)
            Dim tanggalMulai As Date = ICTanggalMulai.Value.ToShortDateString()

            If ((tanggalMulai >= minDateFiskal) And (tanggalMulai <= maxDateFiskal)) Then
                args.IsValid = True
            Else
                cvTanggalMulai.ErrorMessage = "* Tanggal mulai tidak berada didalam tahun fiskal yang dipilih "
                args.IsValid = False
            End If
    ...
    Protected Sub cvTanggalSelesai_ServerValidate(source As Object, args As ServerValidateEventArgs)
        Try
            Dim tanggalSelesai As Date = ICTanggalSelesai.Value.ToShortDateString()
            Dim tanggalMulai As Date = ICTanggalMulai.Value.ToShortDateString()
            Dim tahunFiskal As Integer = CInt(Left(ddlTahunFiscal.SelectedValue.ToString(), 4))
            Dim minDateFiskal As Date = New Date(tahunFiskal, 4, 1)
            Dim maxDateFiskal As Date = New Date(tahunFiskal + 1, 3, 31)
            ...
            If ICTanggalSelesai.Value.Subtract(ICTanggalMulai.Value).Days < 0 Then
                cvTanggalSelesai.ErrorMessage = "* Tgl selesai tidak boleh lebih kecil dari tgl mulai"
                args.IsValid = False
            Else
                If ((tanggalSelesai >= minDateFiskal) And (tanggalSelesai <= maxDateFiskal)) Then
                    args.IsValid = True
                Else
                    cvTanggalSelesai.ErrorMessage = "* Tanggal selesai tidak berada didalam tahun fiskal yang dipilih"
                    args.IsValid = False
                End If
            End If
    ...
    Protected Sub cvConfirmDueDate_ServerValidate(source As Object, args As ServerValidateEventArgs)
        Try
            If ICConfirmDueDate.Value.Subtract(ICTanggalMulai.Value).Days > 0 Then
                cvConfirmDueDate.ErrorMessage = "* Tgl konfirmasi tidak boleh lebih besar dari tgl mulai"
                args.IsValid = False
            End If
        Catch ex As Exception
            args.IsValid = False
