...
Public Class FrmTrClassAllocationNew
...
#Region "Initialize Page Method"
    ...
    Private Sub SetControlPrivilege()
        ...
        btnDownload.Visible = helpers.IsEdit
        btnSetAllocation.Visible = helpers.IsEdit
        trUpload.Visible = helpers.IsEdit
        trTemplateupload.Visible = helpers.IsEdit
    End Sub
...
#Region "Process & Bind Data Method"
    ...
    Private Function GetClass(ByVal year As Integer) As ArrayList
        ...
        criterias.opAnd(New Criteria(GetType(TrClass), "TrCourse.JobPositionCategory.AreaID", MatchType.Exact, Integer.Parse(AreaId)))
        ...
        If txtKodeKelas.Text.Trim().Length > 0 Then
            ...
            criteriasNew.opAnd(New Criteria(GetType(TrClass), "TrCourse.JobPositionCategory.AreaID", MatchType.Exact, Integer.Parse(AreaId)))
    ...
    Private Function GetDataForDataGrid(ByVal IsGetFromDB As Boolean, Optional ByVal isNotZero As Boolean = False) As ArrayList
        'update by 54M
    ...
    Private Sub BindDataGrid(ByVal IsGetFromDB As Boolean, Optional ByVal isNotZero As Boolean = False)
        Dim arrList As ArrayList = GetDataForDataGrid(IsGetFromDB, isNotZero)
        ...
        Next
        'End Optimize
        dtgTrClassAllocation.Visible = True
...
#Region "Validation & Update Form Method"
    ...
    Private Function CheckDataForm(ByRef ErrorMsg As String, Optional ByVal isSetData As Boolean = True) As Boolean
        ...
        If txtKodeKelas.Text.Trim().Length > 0 Then
            ...
            Next i
        Else
            If Not isSetData And txtKodeDealer.IsEmpty Then
                ErrorMsg = SR.GridIsEmpty("Field Kode Dealer & kelas")
                Return False
            End If
        End If
...
#Region "General Method"
    ...
    Private Sub UploadClassAllocation()
        Dim fileName As String = String.Empty
        Dim listAllocation As List(Of TrClassAllocation) = New List(Of TrClassAllocation)
        Dim listError As List(Of ErrorExcelUpload) = New List(Of ErrorExcelUpload)
        Dim resultUpload As String = helpers.UploadFile(fileUpload, "DataTemp", _
                                                    KTB.DNet.Lib.WebConfig.GetValue("MaximumFileSize"), _
                                                    New String() {".xlsx", ".xls"})
        ...
        Using excelPkg As New ExcelPackage(fileInfo)
            Using ws As ExcelWorksheet = excelPkg.Workbook.Worksheets.Item(1)
                ...
                If ColumnCount < 4 Then
                    MessageBox.Show("Format template tidak Sesuai")
                    Exit Sub
                Else
                    Dim isValid As Boolean = True
                    Dim arrColumn() As String = {"kode kategori", "kode kelas", "kode dealer", "alokasi"}
                    For idx As Integer = 0 To arrColumn.Length - 1
                        If Not ws.GetCellValue(3, idx + 1).ToLower().Equals(arrColumn(idx)) Then
                            isValid = False
                            Exit For
                        End If
                    Next
                ...
                For idx As Integer = 4 To RowCount
                    ...
                    Dim kodeTraining As ExcelField = validasi.Create("Kode Kategori", idx, 1, "required")
                    Dim kodeKelas As ExcelField = validasi.Create("Kode Kelas", idx, 2, "required")
                    Dim kodeDealer As ExcelField = validasi.Create("Kode Dealer", idx, 3, "required")
                    Dim alokasi As ExcelField = validasi.Create("Alokasi", idx, 4, "required,numeric")
                    ...
                    Dim objCourse As TrCourse = GetTrCourseByCode(kodeTraining.Value)
                    ...
                    Else
                        If objCourse.JobPositionCategory Is Nothing Then
                            listErrorfield.Add(validasi.CreateCustomError(kodeTraining, "tidak ditemukan."))
                        Else
                            If Not objCourse.JobPositionCategory.AreaID.Equals(CInt(AreaId)) Then
                                listErrorfield.Add(validasi.CreateCustomError(kodeTraining, "tidak ditemukan."))
                            End If
                        End If
                    End If

                    Dim objClass As TrClass = GetTrClassByCode(kodeKelas.Value)
                    If objClass.ID.Equals(0) Then
                        listErrorfield.Add(validasi.CreateCustomError(kodeKelas, "tidak ditemukan."))
                    Else
                        If objClass.TrCourse.JobPositionCategory Is Nothing Then
                            listErrorfield.Add(validasi.CreateCustomError(kodeKelas, "tidak ditemukan."))
                        Else
                            If Not objClass.TrCourse.JobPositionCategory.AreaID.Equals(CInt(AreaId)) Then
                                listErrorfield.Add(validasi.CreateCustomError(kodeKelas, "tidak ditemukan."))
                            End If
                        End If
                    End If

                    Dim objDealer As Dealer = New DealerFacade(User).Retrieve(kodeDealer.Value)
                    ...
                    If listError.Count.Equals(0) Then
                        Dim dataAllo As TrClassAllocation = New TrClassAllocation()
                        dataAllo.TrClass = objClass
                        dataAllo.Dealer = objDealer
                        dataAllo.Allocated = CInt(alokasi.Value)

                        listAllocation.Add(dataAllo)
                    End If
                Next
                ...
                Else
                    ...
                    End If
                    objSessionHelper.SetSession("arlCA", listAllocation)
                    SetCapacityMaster(listAllocation)
                    objSessionHelper.SetSession("arlCA", listAllocation)
                    dtgUpload.DataSource = listAllocation
                    dtgUpload.VirtualItemCount = listAllocation.Count
                    dtgUpload.DataBind()
    ...
    End Sub
    Private Function GetTrCourseByCode(ByVal code As String) As TrCourse
        Try
            Dim crtCA As CriteriaComposite = New CriteriaComposite(New Criteria(GetType(TrCourse), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
            crtCA.opAnd(New Criteria(GetType(TrCourse), "CourseCode", code))
            crtCA.opAnd(New Criteria(GetType(TrCourse), "JobPositionCategory.AreaID", CInt(AreaId)))
            Return New TrCourseFacade(User).Retrieve(crtCA)(0)
        Catch
        End Try
        Return New TrCourse()
    End Function

    Private Function GetTrClassByCode(ByVal code As String) As TrClass
        Try
            Dim crtCA As CriteriaComposite = New CriteriaComposite(New Criteria(GetType(TrClass), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
            crtCA.opAnd(New Criteria(GetType(TrClass), "ClassCode", code))
            crtCA.opAnd(New Criteria(GetType(TrClass), "TrCourse.JobPositionCategory.AreaID", CInt(AreaId)))
            Return New TrClassFacade(User).Retrieve(crtCA)(0)
        Catch
        End Try
        Return New TrClass()
    End Function
...
#End Region
    ...
    Private Sub SetCapacityMaster(ByVal parlCA As ArrayList)
        SetCapacityMaster(parlCA.Cast(Of TrClassAllocation).ToList())
    End Sub

    Private Sub SetCapacityMaster(ByVal parlCA As List(Of TrClassAllocation))
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        helpers.CheckPrivilege("priv7" + PrivArea)
...
#Region "Event Handler "
    ...
    Private Sub dtgTrClass_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgTrClassAllocation.ItemDataBound
        If e.Item.ItemType = ListItemType.Item Or _
            ...
            Dim allAlocated As Integer = objAllocation.AllocatedTaken
    ...
    Private Sub dtgUpload_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgUpload.ItemDataBound
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Or e.Item.ItemType = ListItemType.SelectedItem Then
            ...
            'objC = objCFac.Retrieve(CInt(txtClassID.Text))
            'objD = objDFac.Retrieve(CInt(txtDealerID.Text))
            objCA = CType(Me.objSessionHelper.GetSession("arlCA"), List(Of TrClassAllocation))(e.Item.ItemIndex)
            objC = objCA.TrClass
            objD = objCA.Dealer
            ...
            If e.Item.ItemIndex > 0 Then
                If Not IsNothing(objCA.TrClass) AndAlso Not IsNothing(objCA.Dealer) Then
                    Dim Idx As Integer = 0
                    For Each oCA As TrClassAllocation In CType(Me.objSessionHelper.GetSession("arlCA"), List(Of TrClassAllocation))
    ...
    Private Sub btnCari_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCari.Click
        ...
        Dim strErrorMsg As String = String.Empty
        If CheckDataForm(strErrorMsg, False) Then
            objSessionHelper.RemoveSession("arlAllocation")
...
#End Region
    ...
    Private Sub linkTemplate_Click(sender As Object, e As EventArgs) Handles linkTemplate.Click
        ...
        template.FileName = "TemplateUploadAlokasi.xls"
        template.SheetName = "UploadAlokasi"
        template.Judul = "Upload Alokasi Kelas"
        template.AddField(1, "Kode Kategori")
        template.AddField(2, "Kode Kelas")
        template.AddField(3, "Kode Dealer")
        template.AddField(4, "Alokasi")
        template.DownLoad()
    End Sub
