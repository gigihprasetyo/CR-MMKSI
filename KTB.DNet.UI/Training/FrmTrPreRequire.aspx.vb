...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents hdnCriteria As System.Web.UI.WebControls.HiddenField
...
#Region "Custom Method"
    ...
    Private Sub SetTitleAndHiddenField()
        Select Case Request.QueryString("category").ToString()
            Case "sales"
                lblTitle.Text = "Training Sales - Prasyarat"
                hdnCategory.Value = "sales"
                hdnCriteria.Value = "1"
            Case "ass"
                lblTitle.Text = "Training After Sales - Prasyarat"
                hdnCategory.Value = "ass"
                hdnCriteria.Value = "0,2,3"
            Case "cs"
                lblTitle.Text = "Training Customer Satisfaction - Prasyarat"
    ...
    Private Sub BindDataGrid(ByVal indexPage As Integer)
        ...
        If Not IsNothing(arrPreRequire) Then
            ...
            dtgPreRequire.DataSource = arrPreRequire
            dtgPreRequire.VirtualItemCount = totalRow
    ...
    Private Sub ClearData()
        ...
        lblPopUpCourse2.Enabled = True
        lblPreRequireCodeNoPass.Enabled = True
        ...
        dtgPreRequire.PagerStyle.Visible = True
            ...
            For Each strlulus As String In arrLulus
                ...
                If objCourseId.ID > 0 And objPreReqCourseId.ID > 0 Then
                    If Not IsExistGroup(objCourseId.ID, objPreReqCourseId.ID, EnumCourseRequireType.RequireType.SyaratLulus) Then
                        ...
                        objTrPreRequire.Prerequireduration = ConvertStringToInteger(txtDurasi.Text)
            ...
            For Each strblmlulus As String In arrBlmLulus
                ...
                If objCourseId.ID > 0 And objPreReqCourseId.ID > 0 Then
                    If Not IsExistGroup(objCourseId.ID, objPreReqCourseId.ID, EnumCourseRequireType.RequireType.SyaratBelumLulus) Then
                        ...
                        objTrPreRequire.Description = Me.txtDesc.Text.Trim()
                        objTrPreRequire.Prerequireduration = 0
                        iLulus = New TrPreRequireFacade(User).Insert(objTrPreRequire)
                    Else
    ...
    Private Sub ViewGroup(ByVal nID As Integer, ByVal EditStatus As Boolean)
        ...
        If Not IsNothing(objTrPreRequire) Then
            ...
            txtDurasi.Text = ConvertStringToInteger(objTrPreRequire.Prerequireduration.ToString())
...
#Region "EventHandler"
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load, Me.Load
        ...
        helpers.CheckPrivilege("priv4" + PrivArea)
    ...
    Private Function RetriveDataAndSaveToCache() As Integer
        ...
        If txtDurasi.Text <> String.Empty Then
            criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.TrPreRequire), "Prerequireduration", MatchType.Exact, CType(txtDurasi.Text, Integer)))
        End If

        criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.TrPreRequire), "TrCourse.JobPositionCategory.ID", MatchType.InSet, "(" + hdnCriteria.Value + ")"))

        If DdlStatus.IsSelected Then
            criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.TrPreRequire), "TrCourse.Status", MatchType.Exact, DdlStatus.SelectedValue))
        End If
    ...
    Private Sub SetControlPrivilege()
        m_bFormPrivilege = helpers.IsEdit
    ...
    Private Sub dtgPreRequire_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dtgPreRequire.ItemCommand
        If (e.CommandName = "View") Then
            ...
            lblPopUpCourse2.Enabled = False
            lblPreRequireCodeNoPass.Enabled = False
            dtgPreRequire.PagerStyle.Visible = False
        ...
        ElseIf e.CommandName = "Edit" Then
            ...
            'If lblType.Text = EnumCourseRequireType.GetStringValue(EnumCourseRequireType.RequireType.SyaratLulus) Then
            '    CType(e.Item.FindControl("btnSave"), LinkButton).Visible = m_bFormPrivilege
            'Else
            '    CType(e.Item.FindControl("btnSave"), LinkButton).Visible = False
            'End If
            'Dim RowValue As TrPreRequire = CType(e.Item.DataItem, TrPreRequire)
            'If RowValue.RequireType = EnumCourseRequireType.RequireType.SyaratLulus Then
            '    CType(e.Item.FindControl("btnSave"), LinkButton).Visible = m_bFormPrivilege
            'Else
            '    CType(e.Item.FindControl("btnSave"), LinkButton).Visible = False
            'End If
        ...
        ElseIf e.CommandName = "Save" Then
            Dim txtPrerequireduration As TextBox = e.Item.FindControl("txtPrerequireduration")
            UpdatePrerequreDuration(e.Item.Cells(0).Text, txtPrerequireduration.Text)
        End If
    End Sub

    Private Sub UpdatePrerequreDuration(ByVal id As Integer, ByVal duration As Integer)
        Try

            Dim trPrerequireFacade As TrPreRequireFacade = New TrPreRequireFacade(User)
            Dim data As TrPreRequire = trPrerequireFacade.Retrieve(id)

            If data.Prerequireduration = duration Then
                MessageBox.Show("Tidak ada perubahan data")
                Exit Sub
            End If

            data.Prerequireduration = duration


            Dim result As Integer = trPrerequireFacade.Update(data)

            If result = -1 Then
                Throw New Exception("error")
            End If

            MessageBox.Show("Data Prasyarat berhasil diubah")

        Catch ex As Exception
            MessageBox.Show("Gagal dalam mengupdate Prasyarat")
        End Try
    End Sub
    ...
    Private Sub dtgPreRequire_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgPreRequire.ItemDataBound
        If Not e.Item.DataItem Is Nothing Then
            ...
            Dim txtPrerequireduration As TextBox = e.Item.FindControl("txtPrerequireduration")
            If RowValue.RequireType = EnumCourseRequireType.RequireType.SyaratLulus Then
                CType(e.Item.FindControl("btnSave"), LinkButton).Visible = helpers.IsEdit
                txtPrerequireduration.Visible = True
            Else
                CType(e.Item.FindControl("btnSave"), LinkButton).Visible = False
                txtPrerequireduration.Visible = False
            End If


            If IsDBNull(RowValue.Prerequireduration) Then
                txtPrerequireduration.Text = "0"
            Else
                txtPrerequireduration.Text = RowValue.Prerequireduration
            End If
    ...
    Private Function ConvertStringToInteger(ByVal val As String)
        Try
            Return CInt(val)
        Catch ex As Exception
            Return 0
...
#End Region
    ...
    Protected Sub txtPreRequireCode_TextChanged(sender As Object, e As EventArgs)
        If txtPreRequireCode.Text.Trim().Length = 0 Then
            txtDurasi.Enabled = False
        Else
