...
#Region " subs & function "
    ...
    Private Sub bindDataGrid(ByVal indexPage As Integer)
        ...
        If Not String.IsNullOrEmpty(txtLocation.Text) Then
            criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.TrClass), "LocationName", MatchType.[Partial], txtLocation.Text))
        End If
        criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.TrClass), "TrCourse.JobPositionCategory.JobPositionCategoryArea.ID", MatchType.Exact, "3"))
        ...
        arlResult = New TrClassFacade(User).RetrieveActiveList(criterias, indexPage + 1, dtgTrClass.PageSize, totalRow, CType(ViewState("CurrentSortColumn"), String), CType(ViewState("CurrentSortDirect"), Sort.SortDirection))
        dtgTrClass.DataSource = arlResult
        Session("SessListTrClassCS") = arlResult
...
#Region " control event handler "
    ...
    Private Sub btnBatal_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBatal.Click
        helpers.RemoveSession("dataUpload")
        Response.Redirect("FrmTrClassCs.aspx")
    End Sub
    Private Sub dtgCourse_ItemCommand(ByVal source As System.Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dtgTrClass.ItemCommand
        If (e.CommandName = "View") Then
            ...
            Response.Redirect("FrmTrClassDetailCs.aspx")
    ...
    Private Sub dtgTrClass_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgTrClass.ItemDataBound
        ...
        If Not e.Item.DataItem Is Nothing Then
            ...
            Dim lbtnAddAllocation As LinkButton = CType(e.Item.FindControl("lbtnAddAllocation"), LinkButton)
            Dim btnHapus As LinkButton = CType(e.Item.FindControl("btnHapus"), LinkButton)

            If RowValue.CreatedBy.ToLower = "migration" Then
                btnUbah.Visible = False
                lbtnAddAllocation.Visible = False
    ...
    Private Sub btnDownload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDownload.Click
        Response.Redirect("./FrmDownloadTrClassCs.aspx")
    End Sub
...
#End Region
    ...
    Private Function GetData(ByVal strPeriod As String) As ArrayList
        ...
        criterias = New CriteriaComposite(New Criteria(GetType(KTB.DNet.Domain.TrClass), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
        criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.TrClass), "TrCourse.JobPositionCategory.ID", MatchType.Exact, 4))
    ...
    Private Sub linkTemplate_Click(sender As Object, e As EventArgs) Handles linkTemplate.Click
        ...
        template.AddField(6, "Nama Lokasi")
        template.AddField(7, "Alamat")
        template.AddField(8, "Pengajar 1")
        template.AddField(9, "Pengajar 2")
        template.AddField(10, "Pengajar 3")
        template.AddField(11, "Kapasitas")
        template.AddField(14, "Keterangan")
        template.AddField(15, "Tanggal Mulai")
        template.AddField(16, "Tanggal Selesai")
        template.AddField(17, "Tanggal Batas Konfirmasi")
        ...
        Dim dataTahunfiskal As ExcelTemplateColumn = New ExcelTemplateColumn(12, "Tahun Fiskal", EnumTypeCell.Dropdownlist)
        ...
        Dim dataStatus As ExcelTemplateColumn = New ExcelTemplateColumn(13, "Status", EnumTypeCell.Dropdownlist)
        ...
        Using excelPkg As New ExcelPackage(fileInfo)
            Using ws As ExcelWorksheet = excelPkg.Workbook.Worksheets.Item(1)
                ...
                For idx As Integer = 4 To RowCount
                    ...
                    Dim Kota As ExcelField = validasi.Create("Kota", idx, 5, "required,max", 50)
                    Dim NamaLokasi As ExcelField = validasi.Create("Nama Lokasi", idx, 6, "required,max", 100)
                    Dim Alamat As ExcelField = validasi.Create("Alamat", idx, 7, "required,max", 200)
                    Dim Pengajar1 As ExcelField = validasi.Create("Pengajar 1", idx, 8, "required,max", 50)
                    Dim Pengajar2 As ExcelField = validasi.Create("Pengajar 2", idx, 9, "max", 50)
                    Dim Pengajar3 As ExcelField = validasi.Create("Pengajar 3", idx, 10, "max", 50)
                    Dim Kapasitas As ExcelField = validasi.Create("Kapasitas", idx, 11, "required,numeric")
                    Dim TahunFiscal As ExcelField = validasi.Create("Tahun Fiskal", idx, 12, "required")
                    Dim Status As ExcelField = validasi.Create("Status", idx, 13, "required")
                    Dim Keterangan As ExcelField = validasi.Create("Keterangan", idx, 14, "max", 200)
                    Dim TanggalMulai As ExcelField = validasi.Create("Tanggal Mulai", idx, 15, "required,date")
                    Dim TanggalSelesai As ExcelField = validasi.Create("Tanggal Selesai", idx, 16, "required,date")
                    Dim TanggalKonfirmasi As ExcelField = validasi.Create("Tanggal Batas Konfirmasi", idx, 17, "required,date")
                    ...
                    If TanggalMulai.Value.StringCellToDateTime() > TanggalSelesai.Value.StringCellToDateTime() Then
                        listErrorfield.Add(validasi.CreateCustomError(TanggalMulai, "lebih besar dari Tanggal Selesai"))
                    End If

                    If TanggalKonfirmasi.Value.StringCellToDateTime() > TanggalMulai.Value.StringCellToDateTime() Then
                        listErrorfield.Add(validasi.CreateCustomError(TanggalKonfirmasi, "lebih besar dari Tanggal Mulai"))
                    End If
                    ...
                    Else
                        If objCourse.JobPositionCategory Is Nothing Then
                            listErrorfield.Add(validasi.CreateCustomError(kodeTraining, "tidak ditemukan."))
                        Else
                            If Not objCourse.JobPositionCategory.ID.Equals(4) Then
                                listErrorfield.Add(validasi.CreateCustomError(kodeTraining, "tidak ditemukan."))
                            End If
                        End If
                    End If
                    ...
                    If listError.Count.Equals(0) Then
                        ...
                        objClass.FinishDate = TanggalSelesai.Value.StringCellToDateTime()
                        objClass.StartDate = TanggalMulai.Value.StringCellToDateTime()
                        objClass.ConfirmDueDate = TanggalKonfirmasi.Value.StringCellToDateTime()
