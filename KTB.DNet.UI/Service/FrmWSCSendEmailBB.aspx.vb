...
Public Class FrmWSCSendEmailBB
...
#Region "Custom Method"
    ...
    Private Sub RetrieveMaster()
        If Not IsNothing(Session("WSCHead")) Then
            WSCHead = CType(Session("WSCHead"), WSCHeaderBB)
    ...
    Private Sub SaveToDataBase()
        Dim objWSC As WSCDamageRequestPartBB = New WSCDamageRequestPartBB
        Dim objWSCMaster As WSCHeaderBB = Session("WSCHead")
        Dim objWSCFac As WSCDamageRequestPartBBFacade = New WSCDamageRequestPartBBFacade(User)
        ...
        objWSC.WSCHeaderBB = objWSCMaster
        objWSC.Sender = _cc1
        objWSC.CCSendTo = cc
        objWSC.SendTo = _to
        objWSC.Subject = "Bukti Request WSC" & lblClaimNo.Text
        objWSC.Description = txtDescription.Text.Replace("<BR>", (Chr(13) & Chr(10)).ToString)
        nResult = New WSCDamageRequestPartBBFacade(User).Insert(objWSC)
        If nResult = -1 Then
...
#Region "EventHandler"
    ...
    Private Sub BindToDataGrid()
        WSCHead = CType(Session("WSCHead"), WSCHeaderBB)
        dtgSendEmail.DataSource = WSCHead.WSCDamageRequestPartBBs
        sessionHelper.SetSession("WSCDamageRequestPartBB", WSCHead.WSCDamageRequestPartBBs)
        dtgSendEmail.DataBind()
        If Not WSCHead Is Nothing Then
            Dim arlsendEmail As ArrayList = WSCHead.WSCDamageRequestPartBBs
    ...
    Private Sub dtgSendEmail_EditCommand(ByVal source As System.Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dtgSendEmail.ItemCommand
        If e.CommandName = "No" Then
            arlSendEmail = sessionHelper.GetSession("WSCDamageRequestPartBB")
