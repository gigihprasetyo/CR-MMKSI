...
#Region "EventHandler"
    ...
    Private Sub btnUpload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUpload.Click
        ...
        'checkDealer()
        _sesHelper.RemoveSession("WSCHEADERS")
                ...
                If infWSCData.PostedFile.ContentType = "text/plain" Then
                    ...
                    Try
                        If imp.Start() Then
                            ...
                            objWSCParser = New WSCParser
                            objWSCParser.Dealer = CType(Session("DEALER"), Dealer)

                            CompleteListWSC = CType(objWSCParser.ParsingNoTrasaction(TempFile, User.Identity.Name), ArrayList)
                    ...
                    If (Not CompleteListWSC Is Nothing) AndAlso CompleteListWSC.Count > 0 Then
                        ...
                        _sesHelper.SetSession("WSCHEADERS", CompleteListWSC)
                    ...
                    ViewState.Item("IsErrorInData") = objWSCParser.IsErrorExist()
    ...
    Private Sub dtgWSC_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgWSC.ItemDataBound
        If (e.Item.ItemIndex <> -1) Then
            ...
            'If Not CType(ListWSC.Item(e.Item.ItemIndex), WSCHeader).ChassisMaster Is Nothing Then
            '    e.Item.Cells(4).Text = CType(ListWSC.Item(e.Item.ItemIndex), WSCHeader).ChassisMaster.ChassisNumber
            'Else
            '    e.Item.Cells(4).Text = ""
            'End If
            ...
            it = e.Item
            Dim objWSCHeader As New WSCHeader
            objWSCHeader = CType(e.Item.DataItem, WSCHeader)
    ...
    Private Function isKodeTypeExist(ByVal kode As String, ByVal tipe As String, ByVal wscDetailList As ArrayList, ByVal WorkCode As String) As Integer
        Dim found As Integer = 0
        For Each item As WSCDetail In wscDetailList
    ...
    Private Function ValidateWSC(ByVal wscList As ArrayList) As Boolean
        For Each item As WSCHeader In wscList
            Dim count As Integer = 0
            Dim WSCValidationMsg As String = String.Empty
            Dim isPDIFSPMExist As Boolean = ValidationPDIFSPM(WSCValidationMsg)
            If Not isPDIFSPMExist Then
                Return False
            End If

            For Each detil As WSCDetail In item.WSCDetails
                'Dim lCode As String = String.Empty
                'If Not detil.LaborMaster Is Nothing Then
                '    lCode = detil.LaborMaster.LaborCode
                'End If
                count += isKodeTypeExist(detil.Code, detil.Type, item.WSCDetails, detil.WorkQty)
            Next
            If count > item.WSCDetails.Count Then
                Return False
            End If
        Next
        Return True
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Dim ListWSCHeader As ArrayList
        ListWSCHeader = CType(Session("WSCHEADERS"), ArrayList)
        Dim ObjWSCHeaderFacade As WSCHeaderFacade
        ObjWSCHeaderFacade = New WSCHeaderFacade(User)
        Dim WSCValidationMsg As String = String.Empty
        If ValidateWSC(ListWSCHeader) Then
            If ObjWSCHeaderFacade.InsertWSC(ListWSCHeader) = 0 Then
                MessageBox.Show("WSC berhasil ditambahkan")
                Me.btnSave.Enabled = False
            End If
        Else
            If WSCValidationMsg = String.Empty Then
...
#End Region
    ...
    Private Sub dtgWSC_PageIndexChanged(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridPageChangedEventArgs) Handles dtgWSC.PageIndexChanged
        Dim CompletelistWSC As ArrayList = _sesHelper.GetSession("WSCHEADERS")
    ...
    Private Function CheckOutstandingWSC() As Boolean
        Dim objDealer As Dealer = _sesHelper.GetSession("DEALER")
        ...
        If arl.Count > 0 Then
            Server.Transfer("../FrmAccessDenied.aspx?isEncode=1&mess=" & Server.UrlEncode("Anda tidak dapat mengakses halaman ini.") & "&messDescription=" & Server.UrlEncode("<center>Segera selesaikan dokumen yang berstatus baru</center>") & "")
        End If
    End Function

    Private Function ValidationPDIFSPM(ByRef ValidationMsg As String) As Boolean
        Dim objDealer As Dealer = _sesHelper.GetSession("DEALER")
        ...
        critPDI.opAnd(New Criteria(GetType(PDI), "ChassisMaster.ID", MatchType.Exact, objDealer.ID))
        ...
        critFS.opAnd(New Criteria(GetType(FreeService), "ChassisMaster.ID", MatchType.Exact, objDealer.ID))
        ...
        critPM.opAnd(New Criteria(GetType(PMHeader), "ChassisMaster.ID", MatchType.Exact, objDealer.ID))

        Dim arlPM As ArrayList = New WSCHeaderFacade(User).Retrieve(critPM)
        If arlPM.Count <= 0 Then
