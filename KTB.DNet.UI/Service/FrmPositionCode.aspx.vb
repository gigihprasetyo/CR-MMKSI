...
#Region " private custom variable "
    Private ListCodePosition As ArrayList
#End Region
...
#Region "Custom Method"
    ...
    Private Sub CreateCriteria(ByVal criterias As CriteriaComposite)
        If txtKode.Text.Length > 0 Then
            criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.DeskripsiKodePosisi), "KodePosition", MatchType.Exact, txtKode.Text.Trim))
        End If
        If txtDeskripsi.Text.Length > 0 Then
            criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.DeskripsiKodePosisi), "Description", MatchType.[Partial], txtDeskripsi.Text.Trim))
        End If
    End Sub
    Private Sub BindDataGrid(ByVal indexPage As Integer)
        Dim totRow As Integer = 0
        If (indexPage >= 0) Then
            ListCodePosition = New DeskripsiPositionCodeFacade(User).RetrieveActiveList(CType(sessHelper.GetSession("CRITERIAS"), CriteriaComposite), indexPage + 1, _
        dtgModel.PageSize, totRow, CType(ViewState("CurrentSortColumn"), String), CType(ViewState("CurrentSortDirect"), Sort.SortDirection))
            dtgModel.DataSource = ListCodePosition
    ...
    Private Function InsertModel() As Integer
        Dim objDescFacade As DeskripsiPositionCodeFacade = New DeskripsiPositionCodeFacade(User)
        Dim objDesc As DeskripsiKodePosisi = New DeskripsiKodePosisi
        Dim nResult As Integer

        If objDescFacade.ValidateCode(txtKode.Text) = 0 Then
            objDesc.Status = 0
            objDesc.KodePosition = txtKode.Text
            objDesc.Description = txtDeskripsi.Text
            nResult = New DeskripsiPositionCodeFacade(User).Insert(objDesc)
            If nResult = -1 Then
    ...
    Private Sub UpdateModel()
        Dim objDesc As DeskripsiKodePosisi = CType(Session.Item("vsDesc"), DeskripsiKodePosisi)
        objDesc.Description = txtDeskripsi.Text
        objDesc.Status = 0
        Dim nResult = New DeskripsiPositionCodeFacade(User).Update(objDesc)
    End Sub
    ...
    Private Function CreateCriteriaForCheckRecord(ByVal DomainType As Type, _
        ...
        criterias.opAnd(New Criteria(DomainType, "DeskripsiKodePosisi", MatchType.Exact, DescID))
        Return criterias
    End Function
    ...
    Private Sub DeleteModel(ByVal nID As Integer)
        Dim objDesc As DeskripsiKodePosisi = New DeskripsiPositionCodeFacade(User).Retrieve(nID)
        Dim facade As DeskripsiPositionCodeFacade = New DeskripsiPositionCodeFacade(User)
    ...
    Private Sub ViewDeskripsiKodePosisi(ByVal nID As Integer, ByVal EditStatus As Boolean)
        ...
        Else
            txtKode.Text = objDesc.KodePosition
...
#Region "EventHandler"
    ...
    Private Sub btnCari_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCari.Click
        Dim criterias As New CriteriaComposite(New Criteria(GetType(KTB.DNet.Domain.DeskripsiKodePosisi), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
    ...
    Private Sub ActivateUserPrivilege()
        m_bFormPrivilege = SecurityProvider.Authorize(Context.User, SR.ENHServiceKodePosisiDescEdit_Privilege)
        If Not SecurityProvider.Authorize(Context.User, SR.ENHServiceKodePosisiDescView_Privilege) Then
    ...
    Private Sub dtgModel_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgModel.ItemDataBound
        ...
        If e.Item.ItemIndex <> -1 Then
            Try
                Dim row As DeskripsiKodePosisi = CType(e.Item.DataItem, DeskripsiKodePosisi)
                Dim lblKode As Label = CType(e.Item.FindControl("lblkode"), Label)
                Dim lblStatus As Label = CType(e.Item.FindControl("lblStatus"), Label)

                lblKode.Text = row.KodePosition
                lblStatus.Text = CType(row.Status, EnumJobPositionDescription.JobPosition).ToString
