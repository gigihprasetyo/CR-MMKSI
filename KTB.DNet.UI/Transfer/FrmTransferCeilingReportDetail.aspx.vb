...
Public Class FrmTransferCeilingReportDetail
...
#Region "Variables"
    ...
    Private _sessData As String = "FrmTransferCeilingReportDetail._sessData"
...
#Region "Event"
    ...
    Private Sub dtgMain_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles dtgMain.ItemCommand
        ...
        If cmdName.StartsWith("day") Then
            ...
            cTC.opAnd(New Criteria(GetType(TransferCeiling), "ProductCategory.ID", MatchType.Exact, Me.ddlProductCategory.SelectedValue))
            cTC.opAnd(New Criteria(GetType(TransferCeiling), "CreditAccount", MatchType.Exact, Me.txtCreditAccount.Text))
            cTC.opAnd(New Criteria(GetType(TransferCeiling), "PaymentType", MatchType.Exact, Me.ddlPaymentType.SelectedValue))
            cTC.opAnd(New Criteria(GetType(TransferCeiling), "EffectiveDate", MatchType.Exact, dtTime))

            aTCs = oTCFac.Retrieve(cTC)
            If aTCs.Count > 0 Then
                oTC = aTCs(0)

                Response.Redirect("FrmTransferCeilingReportDetail2.aspx?ID=" & oTC.ID.ToString())
            End If
        End If
    End Sub
...
#End Region
    ...
    Protected Sub btnBack_Click(sender As Object, e As EventArgs) Handles btnBack.Click
        Response.Redirect("FrmTransferCeilingReport.aspx?IsBack=1")
    End Sub
End Class