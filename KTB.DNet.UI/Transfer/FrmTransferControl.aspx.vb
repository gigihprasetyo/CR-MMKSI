...
Public Class FrmTransferControl
...
#Region "Variables"
    ...
    Private _sessTransferControls As String = "FrmTransferControl._sessTransferControls"

    Private _vstCritProductCategory As String = "FrmTransferControl._vstCritProductCategory"
    Private _vstCritCreditAccount As String = "FrmTransferControl._vstCritCreditAccount"
    Private _vstCritValidFrom As String = "FrmTransferControl._vstCritValidFrom"
    Private _vstCritPaymentType As String = "FrmTransferControl._vstCritPaymentType"
    Private _vstCritPaymentMethod As String = "FrmTransferControl._vstCritPaymentMethod"
...
#Region "Custom"
    ...
    Private Function GetData() As ArrayList
        ...
        If CType(Me.ddlProductCategory.SelectedValue, Short) <> -1 Then
            cTC.opAnd(New Criteria(GetType(TransferControl), "ProductCategory.ID", MatchType.Exact, CType(Me.ddlProductCategory.SelectedValue, Short)))
        End If

        If Me.txtCreditAccount.Text.Trim <> "" Then
            Dim sCA As String = "'" & Me.txtCreditAccount.Text.Trim.Replace(";", "','") & "'"

            cTC.opAnd(New Criteria(GetType(TransferControl), "CreditAccount", MatchType.InSet, "(" & sCA & ")"))
        End If

        If chkvalidFrom.Checked Then
            cTC.opAnd(New Criteria(GetType(TransferControl), "ValidFrom", MatchType.GreaterOrEqual, Me.calValidFrom.Value.ToString("yyyy.MM.dd")))
        End If

        If chkValidTo.Checked Then
            cTC.opAnd(New Criteria(GetType(TransferControl), "ValidTo", MatchType.GreaterOrEqual, Me.calValidTo.Value.ToString("yyyy.MM.dd")))
        End If

        If chkValidityDate.Checked Then
            cTC.opAnd(New Criteria(GetType(TransferControl), "ValidityDate", MatchType.GreaterOrEqual, Me.calValidityDate.Value.ToString("yyyy.MM.dd")))
        End If

        If CType(Me.ddlPaymentType.SelectedValue, Short) <> -1 Then
            Dim pt As Short = CType(Me.ddlPaymentType.SelectedValue, Short)

            cTC.opAnd(New Criteria(GetType(TransferControl), "PaymentType", MatchType.Exact, pt))
        ...
        If CType(Me.ddlPaymentMethod.SelectedValue, Short) <> -1 Then
            ...
            cTC.opAnd(New Criteria(GetType(TransferControl), "Status", MatchType.Exact, state))
        End If
        sTC.Add(New Sort(GetType(TransferControl), "ProductCategory.Code", SortDirection.Ascending))
        sTC.Add(New Sort(GetType(TransferControl), "CreditAccount", SortDirection.Ascending))
        sTC.Add(New Sort(GetType(TransferControl), "PaymentType", SortDirection.Ascending))
    ...
    Private Sub BindData(ByVal oTC As TransferControl)
    ...
    Private Sub SaveData()
        Dim oTC As New TransferControl
        Dim oTCFac As New TransferControlFacade(User)
        ...
        If CType(Me.ddlProductCategory.SelectedValue, Short) = -1 Then
            MessageBox.Show("Kategori Produk Harus Diisi!")
            Exit Sub
        End If
        If Me.txtCreditAccount.Text.Trim() = "" Then
        ...
        oPC = New ProductCategoryFacade(User).Retrieve(CType(Me.ddlProductCategory.SelectedValue, Integer))

        oTC.ID = Me.txtID.Text
        oTC.ProductCategory = oPC
        ...
        Else
            Dim oTCDB As TransferControl = oTCFac.Retrieve(oTC.ID)
    ...
    Private Sub SaveDataNew()
        ...
        Else
            ...
            If ViewState("IsNew") Then
                ...
                Dim cTC As New CriteriaComposite(New Criteria(GetType(TransferControl), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
                cTC.opAnd(New Criteria(GetType(TransferControl), "ProductCategory.ID", MatchType.Exact, CType(Me.ddlProductCategory.SelectedValue, Short)))
                cTC.opAnd(New Criteria(GetType(TransferControl), "CreditAccount", MatchType.Exact, oVCA.CreditAccount))
                cTC.opAnd(New Criteria(GetType(TransferControl), "PaymentType", MatchType.Exact, ddlPaymentType.SelectedValue))
            ...
            Else
                ...
                cTC.opAnd(New Criteria(GetType(TransferControl), "CreditAccount", MatchType.Exact, oVCA.CreditAccount))
                cTC.opAnd(New Criteria(GetType(TransferControl), "PaymentType", MatchType.Exact, ddlPaymentType.SelectedValue))
                cTC.opAnd(New Criteria(GetType(TransferControl), "ID", MatchType.No, txtID.Text))
...
#Region "Event"
    ...
    Private Sub dtgMain_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles dtgMain.ItemCommand
        Select Case e.CommandName.Trim().ToLower()
            Case "Edit".ToLower()
                ...
                Dim oTC As TransferControl = New TransferControlFacade(User).Retrieve(ID)
    ...
    Private Sub dtgMain_ItemDataBound(sender As Object, e As DataGridItemEventArgs) Handles dtgMain.ItemDataBound
        ...
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            Dim oTC As TransferControl = e.Item.DataItem
            ...
            lblProductCategory.Text = oTC.ProductCategory.Code
            lblCreditAccount.Text = oTC.CreditAccount
            lblPaymentType.Text = CType(oTC.PaymentType, enumPaymentType.PaymentType).ToString()

            lblDeskripsi.Text = oTC.ProductCategory.Code & " - " & oTC.CreditAccount & " - " & CType(oTC.PaymentType, enumPaymentType.PaymentType).ToString()
            ...
            lblUpdatedTime.Text = oTC.LastUpdatedTime.ToString("dd MMM yyyy HH:mm:ss")
            lblUpdatedBy.Text = oTC.LastUpdatedBy
