...
Public Class FrmEventJVList
    Inherits System.Web.UI.Page

    Private oDealer As Dealer
    Private sessHelper As New SessionHelper
    Private arlCheckedItemColl As ArrayList = New ArrayList
    Private SessionCriteriaBabit = "FrmEventJVList.CriteriaList"
    Private PageSession = "FrmEventJVList"
    Private GridIndexSessionBefore = "FrmEventJVList_PgIdxBefore"
    Private GridIndexSessionNext = "FrmEventJVList_PgIdxNext"
    ...
    Private Sub Authorization()
        If Not SecurityProvider.Authorize(Context.User, SR.EVENT_Daftar_Pencairan_Event_Detail_Privilege) Then
            Server.Transfer("../FrmAccessDenied.aspx?modulName=EVENT - DAFTAR PENCAIRAN EVENT")
        Else
            displayPriv = SecurityProvider.Authorize(Context.User, SR.EVENT_Daftar_Pencairan_Event_Detail_Privilege)
            editPriv = SecurityProvider.Authorize(Context.User, SR.EVENT_Daftar_Pencairan_EVENT_Edit_Privilege)
            transferSAPPriv = SecurityProvider.Authorize(Context.User, SR.EVENT_Daftar_Pencairan_EVENT_TransferSAP_Privilege)
        End If
    ...
    Private Sub BindGrid(ByVal pageIndex As Integer)
        ...
        Dim sortColl As SortCollection = New SortCollection
        sortColl.Add(New Sort(GetType(BabitEventReportJV), CType(ViewState("currSortColumn"), String), CType(ViewState("currSortDirection"), Sort.SortDirection)))

        'Dim listSource As ArrayList = New BabitEventReportJVtoReceiptFacade(User).Retrieve(crit, sortColl)
        Dim listSource As ArrayList = New BabitEventReportJVFacade(User).Retrieve(crit, sortColl)
    ...
    Private Function SearchCriteria() As CriteriaComposite
        ...
        If txtKodeDealer.Text <> "" Then
            crit.opAnd(New Criteria(GetType(BabitEventReportJV), "Dealer.DealerCode", MatchType.Partial, txtKodeDealer.Text))
        End If

        If Not oDealer.Title = EnumDealerTittle.DealerTittle.KTB Then
            crit.opAnd(New Criteria(GetType(BabitEventReportJV), "Dealer.DealerCode", MatchType.Partial, lblKodeDealer.Text.Trim.Split("/")(0).Trim))
        End If

        If txtNoJV.Text <> "" Then
            crit.opAnd(New Criteria(GetType(BabitEventReportJV), "RegNumber", MatchType.Partial, txtNoJV.Text))
        End If

        'TODO
        If ddlStatus.SelectedIndex <> 0 Then
            crit.opAnd(New Criteria(GetType(BabitEventReportJV), "Status", MatchType.Exact, ddlStatus.SelectedValue))
        End If
    ...
    Private Function GetCheckedItem() As ArrayList
        ...
        For Each dtgItem As DataGridItem In dgListJV.Items
            ...
            Dim objCM As BabitEventReportJV = CType(arrGrid(nIndeks), BabitEventReportJV)
    ...
    Protected Sub dgListJV_ItemDataBound(sender As Object, e As DataGridItemEventArgs) Handles dgListJV.ItemDataBound
        Dim objBR As BabitEventReportJV = New BabitEventReportJV
        ...
        If e.Item.ItemType = ListItemType.Item OrElse e.Item.ItemType = ListItemType.AlternatingItem Then
            Dim oData As BabitEventReportJV = CType(e.Item.DataItem, BabitEventReportJV)
            ...
            For Each oDF As BabitEventReportJV In arrGridDF
            ...
            'If login dealer then button edit is disable
            If oDealer.Title <> EnumDealerTittle.DealerTittle.KTB Then
                lnkbtnEdit.Visible = False
            End If
        End If
    ...
    Private Sub SetDownload()
        ...
        arrData = New BabitEventReportJVFacade(User).Retrieve(crits)
        If arrData.Count > 0 Then
            CreateExcel("Daftar Pencairan Event", arrData)
        End If
    ...
    Private Sub CreateExcel(ByVal FileName As String, ByVal Data As ArrayList)
        ...
        Using pck As New ExcelPackage()
            ...
            For i As Integer = 0 To Data.Count - 1
                Dim oBabitEventReportJV As BabitEventReportJV = Data(i)
                ws.Cells(i + 4, 1).Value = i + 1
                'Select Case oBabitEventReportJV.Status
                '    Case 0
                ...
                'End Select
                ws.Cells(i + 4, 2).Value = CommonFunction.GetEnumDescription(oBabitEventReportJV.Status, "EnumBabit.EventJVStatus")

                If IsNothing(oBabitEventReportJV.Dealer) Then
                    ws.Cells(i + 4, 3).Value = ""
                    ws.Cells(i + 4, 4).Value = ""
                Else
                    ws.Cells(i + 4, 3).Value = oBabitEventReportJV.Dealer.DealerCode
                    ws.Cells(i + 4, 4).Value = oBabitEventReportJV.Dealer.DealerName
                End If

                ws.Cells(i + 4, 5).Value = oBabitEventReportJV.RegNumber
                ws.Cells(i + 4, 6).Value = oBabitEventReportJV.TextRefNo
                ws.Cells(i + 4, 7).Value = Format(oBabitEventReportJV.TglPencairan, "dd/MM/yyyy").ToString
                If oDealer.Title = EnumDealerTittle.DealerTittle.KTB Then
                    ws.Cells(i + 4, 8).Value = Format(oBabitEventReportJV.TglProses, "dd/MM/yyyy").ToString
                    ws.Cells(i + 4, 9).Value = oBabitEventReportJV.NoJV
                End If
            Next
    ...
    Protected Sub btnTransfer_Click(sender As Object, e As EventArgs) Handles btnTransfer.Click, btnTransferUlang.Click
        ...
        '1:.Update BabitEventReportJV.Status = 1
        '2:.Update BabitEventReportJV.IsTransfer = 1
        '3:.Update BabitEventReportJV.TglProses = getdate()
        '4:.Update BabitEventReportJV.LastUpdateBy & BabitEventReportJV.LastUpdateTime
        ...
        If CType(sender, Button).ClientID = "btnTransfer" Then
            For Each bJV As BabitEventReportJV In arrchk
        ...
        Dim sbDealerBankAccount As StringBuilder = New StringBuilder
        Dim errFlag As Boolean = False
        Dim arrSuccess As New ArrayList

        For Each oBJV As BabitEventReportJV In arrchk
            Dim jvToRec As BabitEventReportJVtoReceipt = New BabitEventReportJVtoReceiptFacade(User).RetrieveByBabitEventReportJV(oBJV.ID)
            errFlag = False
            ...
            If oBJV.TextRefNo.Trim = "" Then
                ...
                errFlag = True
            End If
            If IsNothing(jvToRec.BabitEventReportReceipt.DealerBankAccount) Then
                'If IsNothing(oBJV.DealerBankAccount) Then
                sbDealerBankAccount.Append(oBJV.RegNumber & ", ")
                errFlag = True
            End If
        ...
        End If
        If sbDealerBankAccount.ToString().Trim <> "" Then
            sb.Append("- Nomor Rekening masih kosong untuk Nomor JV: " & Left(sbDealerBankAccount.ToString().Trim, Len(sbDealerBankAccount.ToString().Trim) - 1) & ". Silahkan input Nomor Rekening terlebih dahulu\n\n")
        End If
    ...
    Private Sub WriteEventJV(arrSuccess As ArrayList)
        ...
        For Each oBJV As BabitEventReportJV In arrSuccess
            Dim line As New System.Text.StringBuilder
            Dim item As BabitEventReportJVtoReceipt = New BabitEventReportJVtoReceiptFacade(User).RetrieveByBabitEventReportJV(oBJV.ID)

            If IsNothing(item.BabitEventReportReceipt.DealerBankAccount) Then
                'If IsNothing(item.BabitEventReportJV.DealerBankAccount) Then
                errorMes = "Tolong cek kembali nomor rekening dealer: " & item.BabitEventReportJV.Dealer.DealerCode & "\n"
                Continue For
            End If

            Dim FakturPajakDate As Date = Date.Now
            Dim ReceiptDate As Date = Date.Now
            Dim q As New BabitEventReportJVtoReceiptFacade(User)
            q.RetrieveOldestDate(oBJV.ID, FakturPajakDate, ReceiptDate)
            Dim Total As Decimal = 0
            If Not item Is Nothing Then
                line.Append("H")
                line.Append(separator)
                line.Append(item.BabitEventReportJV.Dealer.DealerCode)
                line.Append(separator)
                line.Append(item.BabitEventReportJV.RegNumber)
                line.Append(separator)
                line.Append(FakturPajakDate.ToString("yyyyMMdd"))
                line.Append(separator)
                line.Append(ReceiptDate.ToString("yyyyMMdd"))
                line.Append(separator)
                line.Append(item.BabitEventReportJV.TextReceiptNo)
                line.Append(separator)
                'line.Append(item.BabitEventReportJV.DealerBankAccount.BankAccount)
                line.Append(item.BabitEventReportReceipt.DealerBankAccount.BankAccount)
                line.Append(separator)
                line.Append(item.BabitEventReportJV.TextRefNo)
                line.Append(separator)

                line.Append(vbNewLine)

                Dim arrDetail As ArrayList = New BabitEventReportJVtoReceiptFacade(User).RetrieveArrBabitEventReportReceipt(oBJV.ID)
                Dim indexMCC As Integer = 0
                For Each detail As BabitEventReportJVtoReceipt In arrDetail
                    line.Append("D")
                    line.Append(separator)
                    line.Append("ACC")
                    line.Append(separator)
                    ...
                    critMCC.opAnd(New Criteria(GetType(MasterCostCenter), "Value", MatchType.Exact, detail.BabitEventReportReceipt.BabitEventReportHeader.BabitEventProposalHeader.EventDealerHeader.Category.CategoryCode))
                    critMCC.opAnd(New Criteria(GetType(MasterCostCenter), "Type", MatchType.Exact, 2))   'untuk type Event
                    ...
                    Dim MCC As New MasterCostCenter
                    If arrMCC.Count > 0 Then
                        MCC = CType(arrMCC(0), MasterCostCenter)
                    End If
                    line.Append(MCC.CostCenterCode)
                    line.Append(separator)
                    line.Append(detail.BabitEventReportReceipt.MasterAccrued.BussinessAreaCode)
                    line.Append(separator)

                    Dim ioValue As String = "Event"
                    ...
                    critMIO.opAnd(New Criteria(GetType(MasterInternalOrder), "Type", MatchType.Exact, 2))  ''untuk type Event
                    ...
                    line.Append(CInt(detail.BabitEventReportReceipt.ClaimAmount))
                    Total += detail.BabitEventReportReceipt.ClaimAmount
                    endDetail1 = "By. EVENTTERPADU: "

                    line.Append(separator)
                    line.Append(detail.BabitEventReportReceipt.MasterAccrued.AccKey)
                    line.Append(separator)
                    line.Append(endDetail1 & detail.BabitEventReportReceipt.BabitEventReportHeader.ApprovalNumber.Replace(Chr(160), Chr(32)))
                    line.Append(vbNewLine)
                    ...
                    line.Append("VAT")
                    line.Append(separator)
                    line.Append(CInt(detail.BabitEventReportReceipt.VATTotal))

                    line.Append(separator)
                    line.Append(detail.BabitEventReportReceipt.FakturPajakNo)
                    line.Append(separator)
                    line.Append(endDetail1 & detail.BabitEventReportReceipt.BabitEventReportHeader.ApprovalNumber.Replace(Chr(160), Chr(32)))
                    line.Append(vbNewLine)
        ...
        Else
            For Each oBJV As BabitEventReportJV In arrSuccess
                Dim item As BabitEventReportJVtoReceipt = New BabitEventReportJVtoReceiptFacade(User).RetrieveByBabitEventReportJV(oBJV.ID)
                If Not item Is Nothing Then
                    _return = New BabitEventReportJVFacade(User).Update(oBJV)
                End If
            Next
    ...
    Private Sub DoSendSAP(ByVal lines As StringBuilder)
        ...
        Dim ClaimDataPath As String = KTB.DNet.Lib.WebConfig.GetValue("SAPServerFolder") & "\FinishUnit\babit\JVEVTCMPGN" & datetimenow & ".txt"
    ...
    Protected Sub dgListJV_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles dgListJV.ItemCommand
        Select Case e.CommandName
            Case "Detail"
                Response.Redirect("FrmEventJV.aspx?Mode=Detail&BabitEventReportJVID=" & e.Item.Cells(0).Text)
            Case "Edit"
                Response.Redirect("FrmEventJV.aspx?Mode=Edit&BabitEventReportJVID=" & e.Item.Cells(0).Text)
        End Select
    End Sub
