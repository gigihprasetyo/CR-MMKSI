...
Public Class FrmExpenseService
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not IsPostBack Then
            ...
            Else
                ...
                'Try
                '    Dim objAssistLog As AssistUploadLog = New AssistUploadLogFacade(User).Retrieve(CInt(hAssistUploadLogID.Value))
...
#Region "Custom"
    ...
    Private Sub ActivateUserPrivilege()
        ...
        If Not SecurityProvider.Authorize(Context.User, SR.AfterSales_DaftarUpload_View_Privilege) Then
            Server.Transfer("../FrmAccessDenied.aspx?modulName=Data Upload - Expense Service")
        End If
    ...
    Private Sub BindDataGrid(ByVal indexPage As Integer)
        ...
        If (indexPage >= 0) Then
            ...
            Dim criterias As New CriteriaComposite(New Criteria(GetType(KTB.DNet.Domain.AssistExpenseService), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))

            If Not hAssistUploadLogID.Value = "" Then 'jika dibuka dari daftar upload
                criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.AssistExpenseService), "AssistUploadLog.ID", MatchType.Exact, hAssistUploadLogID.Value))
            Else
                If txtKodeDealer.Text.Trim() <> "" Then
                    Dim strKodeDealerIn As String = "('" & txtKodeDealer.Text.Trim().Replace(";", "','") & "')"
                    criterias.opAnd(New Criteria(GetType(AssistExpenseService), "AssistUploadLog.Dealer.DealerCode", MatchType.InSet, strKodeDealerIn))
                End If

                If ddlMonth.SelectedValue > 0 Then
                    criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.AssistExpenseService), "AssistUploadLog.Month", MatchType.Exact, ddlMonth.SelectedValue()))
                End If

                If ddlYear.SelectedValue > 0 Then
                    criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.AssistExpenseService), "AssistUploadLog.Year", MatchType.Exact, ddlYear.SelectedValue()))
                End If

                criterias.opAnd(New Criteria(GetType(KTB.DNet.Domain.AssistExpenseService), "StatusAktif", MatchType.Exact, 1))
            ...
            ListModule = New AssistExpenseServiceFacade(User).RetrieveActiveList(criterias, indexPage + 1, _
            ...
            Dim ObjTargetDealer = (From ObjES As AssistExpenseService In ListModule
            Where ObjES.ExpenseGroup = "TARGET DEALER"
            Order By ObjES.ID
            Select ObjES).ToList()

            Dim ObjBiayaKaryawan = (From ObjES As AssistExpenseService In ListModule
            Where ObjES.ExpenseGroup = "BIAYA KARYAWAN"
            Order By ObjES.ID
            Select ObjES).ToList()

            Dim ObjBiayaOperasional = (From ObjES As AssistExpenseService In ListModule
            Where ObjES.ExpenseGroup = "BIAYA OPERASIONAL"
            Order By ObjES.ID
            Select ObjES).ToList()

            Dim ObjBiayaPerawatan = (From ObjES As AssistExpenseService In ListModule
            Where ObjES.ExpenseGroup = "BIAYA PERAWATAN"
            Order By ObjES.ID
            Select ObjES).ToList()

            Dim ObjBiayaPenyusutan = (From ObjES As AssistExpenseService In ListModule
            Where ObjES.ExpenseGroup = "BIAYA PENYUSUTAN"
            Order By ObjES.ID
            Select ObjES).ToList()

            Dim ObjBiayaUmum = (From ObjES As AssistExpenseService In ListModule
    ...
    Private Sub btnDownload_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDownload.Click
        If (hQuery.Value <> "") Then
            ...
            url = url.Replace("[ASSISTMODULETYPE]", "EXPENSESERVICE")
    ...
    Private Sub dtgBiayaKaryawan_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgBiayaKaryawan.ItemDataBound
        ...
        If Not e.Item.DataItem Is Nothing Then
            Dim RowValue As AssistExpenseService = CType(e.Item.DataItem, AssistExpenseService)
