...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents TAFLinkButton As System.Web.UI.WebControls.LinkButton
    Protected WithEvents imgTAF As System.Web.UI.HtmlControls.HtmlImage
    Protected WithEvents lnkMaintenance As System.Web.UI.WebControls.LinkButton
...
#Region "Private Declaration"
    Private containerPage As default_training
...
#Region "Public Declaration"
    Public MainFrameSrc As String = "Welcome.aspx?modcat=general"
#End Region
...
#Region "Private Methode"
    ...
    Private Sub DisplayTreeMenu()
        If Not Request.QueryString("screenid") = Nothing Then
            ...
            If Not CommonFunction.ValidPage(Request.QueryString("screenid"), objUser.Dealer, reason) Then
                containerPage.MainFrameSrc = "FrmInvalidMenuControl.aspx?reason=" & reason.ToString
            Else
    ...
    Private Sub SetMainFramePage()
        Dim objDealer As Dealer = sessionHelper.GetSession("DEALER")
        Dim _type As String = Request.QueryString("type")
        If Not objDealer Is Nothing Then
            Dim objUserInfo As UserInfo = sessionHelper.GetSession("LOGINUSERINFO")
            If objUserInfo.ID > 0 Then
                If _type = "0" Then
                    If objUserInfo.LoginFlag = "0" Then
                        MainFrameSrc = "UserManagement/frmUserInfoDetail.aspx?type=0"
                    End If
                ElseIf _type = "1" Then
                    MainFrameSrc = "UserManagement/frmUserInfoDetail.aspx?type=1"
                End If
            Else
    ...
    Private Sub Initialization()
        ...
        If Not objDealer Is Nothing Then
            ...
            If objDealer.Title = CType(EnumDealerTittle.DealerTittle.KTB, String) Then
                SetTreeMenu("TreeMenu.XML.PathFromRootTrainingKTB", 10000)
            Else
                If objDealer.Title = CType(EnumDealerTittle.DealerTittle.DEALER, String) Then
                    SetTreeMenu("TreeMenu.XML.PathFromRootTrainingDealer", 10000)
                Else
                    SetTreeMenu("TreeMenu.XML.PathFromRootTrainingDealer", 10000)
                End If
    ...
    Private Sub SetChecklistMaintenance()
        Dim objUI As UserInfo = sessionHelper.GetSession("LOGINUSERINFO")
        ...
        sessionHelper.SetSession("Checklist.SMSNumber", "")
        sessionHelper.SetSession("Checklist.EmailAddress", "")
        ...
        For i = 0 To arrUser.Length - 1
            ...
            If arrProp(0).ToUpper = objUI.UserName.ToUpper And arrProp(1).ToUpper = objUI.Dealer.DealerCode.ToUpper Then
                sessionHelper.SetSession("Checklist.SMSNumber", arrProp(2))
                sessionHelper.SetSession("Checklist.EmailAddress", arrProp(3))
                IsAdmin = True
                Exit For
            End If
        Next
        lnkMaintenance.Visible = IsAdmin
    End Sub
...
#Region "Page Event"
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not Page.IsPostBack Then
            ...
            TAFLinkButton.Visible = False ' CType(KTB.DNet.Lib.WebConfig.GetValue("IsLeasingAvailable"), Boolean)
            imgTAF.Visible = False 'CType(KTB.DNet.Lib.WebConfig.GetValue("IsLeasingAvailable"), Boolean)
    ...
    Private Sub GeneralLinkButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles GeneralLinkButton.Click
        ...
        If Not objDealer Is Nothing Then
            If objDealer.Title = CType(EnumDealerTittle.DealerTittle.KTB, String) Then
                SetTreeMenu("TreeMenu.XML.PathFromRootGeneral", 2300)
            Else
                If objDealer.Title = CType(EnumDealerTittle.DealerTittle.DEALER, String) Then
                    SetTreeMenu("TreeMenu.XML.PathFromRootGeneralDealer", 2300)
                Else
                    SetTreeMenu("TreeMenu.XML.PathFromRootGeneralLeasing", 2300)
                End If
    ...
    Private Sub doLogout()
        LogTosyslog("user logout successfully", "logout-general", "success")
        Session.RemoveAll()
...
#End Region
    ...
    Private Sub TAFLinkButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TAFLinkButton.Click
        sessionHelper.RemoveAllExceptLoginData()
        Response.Redirect("default_TAF.aspx")
    End Sub

    Private Sub lnkMaintenance_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lnkMaintenance.Click
        MainFrameSrc = "General/FrmChecklistMaintenance.aspx"
    End Sub
