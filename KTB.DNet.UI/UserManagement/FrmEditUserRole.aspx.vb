...
#End Region
        ...
        If Not IsPostBack Then
            If IsNothing(Request.QueryString("UserID")) = False Then
                ...
                If sessDealer.Title = EnumDealerTittle.DealerTittle.KTB Then
                    If SecurityProvider.Authorize(Context.User, SR.AdminUpdateRoleKTB_Privilege) Then
                    ElseIf SecurityProvider.Authorize(Context.User, SR.Admin_lihat_role_ktb_privilege) Then
                        For Each di As DataGridItem In Me.dtgUserRole1.Items
                            Dim cbItem As CheckBox = di.FindControl("cbItem")
                            Dim cbInitial As CheckBox = di.FindControl("cbInitial")
                            cbItem.Enabled = False
                            cbInitial.Enabled = False
                        Next
    ...
    Sub BindUserRole(ByVal Obj As UserInfo)
        ...
        sessionHelper.SetSession("InitialRole", coll)
    ...
    Private Sub btnSimpan_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSimpan.Click
        ...
        If IsNothing(Session("sesRoleOld")) = False Then
            ...
            If nResult = 1 Then
                ...
                LogChanges(ObjUserAddRole)
                'For Each objRole As UserRole In sessionHelper.GetSession("InitialRole")
    ...
    Private Sub LogChanges(ByVal ObjUserAddRole As UserInfo)

        Dim Removedrole As String = ""
        Dim Addedrole As String = ""

        For Each item As DataGridItem In dtgUserRole1.Items
            Dim cbItem As CheckBox = item.FindControl("cbItem")
            Dim cbInitial As CheckBox = item.FindControl("cbInitial")
            If cbItem.Checked <> cbInitial.Checked Then
                Dim lblroleName As Label = item.FindControl("lblroleName")
                If cbItem.Checked Then
                    Addedrole += lblroleName.Text & ","
                Else
                    Removedrole += lblroleName.Text & ","
                End If
            End If
        ...
        If Addedrole <> "" Then
            Addedrole = Left(Addedrole, Addedrole.Length - 1)
            LogTosyslog("role added for user " & ObjUserAddRole.Dealer.DealerCode & "-" & ObjUserAddRole.UserName & " are : " & Addedrole, "role-added", "success", "web-security", "add")
        End If
        If Removedrole <> "" Then
            Removedrole = Left(Removedrole, Removedrole.Length - 1)
            LogTosyslog("role removed for user " & ObjUserAddRole.Dealer.DealerCode & "-" & ObjUserAddRole.UserName & " are : " & Removedrole, "role-removed", "success", "web-security", "delete")
        End If
    ...
    Private Sub dtgUserRole1_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgUserRole1.ItemDataBound
        ...
        End If
    End Sub
    'Private Sub LogTosyslog()
    '    Dim strLog As Boolean = KTB.DNet.Lib.WebConfig.GetValue("EnableSyslog")
    '    If strLog Then
    '        Dim objSyslog As SyslogParameter = New SyslogParameter(User)
    '        Dim m As KTB.DNet.Lib.SysLogXMLMessage = New KTB.DNet.Lib.SysLogXMLMessage
    '        m.Action = "Change User Role"
    '        m.SubBlockName = "User Management"
    '        m.FullMessage = "Change User Role " & User.Identity.Name
