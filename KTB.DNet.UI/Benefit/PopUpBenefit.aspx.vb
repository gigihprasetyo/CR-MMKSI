...
#End Region
    ...
    Private Sub BindResult(ByVal indexPage As Integer)
        ...
        If Not dimDealer = Nothing And Not dimDealer = "" Then
            ...
            strSql += " select a.ID from BenefitMasterHeader a inner join BenefitMasterDealer b "
            strSql += " on a.ID = b.BenefitMasterHeaderID  and b.rowstatus = 0 "
            strSql += " inner join Dealer c on b.DealerID = c.ID  and c.rowstatus = 0 "

            If Not Request.QueryString("event") = Nothing Then
                strSql += " left join   BenefitMasterDetail d on a.ID = d.BenefitMasterHeaderID and d.rowstatus = 0 "
                strSql += " left join   BenefitType e on d.BenefitTypeID = e.id and e.rowstatus = 0 "
            End If

            strSql += " where c.DealerCode in ('" & dimDealer.Replace(";", "','") & "') and a.rowstatus = 0 "

            If Not Request.QueryString("event") = Nothing Then
                strSql += " and e.eventvalidation = 1 "
                strSql += " and a.ID not in ("
                strSql += " select BenefitMasterHeaderID from BenefitEventHeader aa"
                strSql += " inner join Dealer bb on aa.dealerid = bb.ID "
                'strSql += " where bb.DealerCode = '" & dimDealer.Replace(";", "','") & "' and aa.RowStatus = 0 ) "
                strSql += " where bb.DealerCode IN ( '" & dimDealer.Replace(";", "','") & "' ) and aa.RowStatus = 0 and aa.Status <> 5) "
            End If
