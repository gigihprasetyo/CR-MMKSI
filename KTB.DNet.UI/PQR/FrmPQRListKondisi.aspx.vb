...
Public Class FrmPQRListKondisi
...
#End Region
    ...
    Sub BindToGrid(ByVal currentPageIndex As Integer, Optional ByVal IsForDownloadPurpose As Boolean = False)
        ...
        criterias.opAnd(New Criteria(GetType(PQRHeader), "ValidationTime", MatchType.GreaterOrEqual, icTglValidateDari.Value))
        criterias.opAnd(New Criteria(GetType(PQRHeader), "ValidationTime", MatchType.LesserOrEqual, icTglValidateSampai.Value.AddDays(1)))
        'End If
        ...
        If txtNoChasis.Text <> String.Empty Then criterias.opAnd(New Criteria(GetType(PQRHeader), "ChassisMaster.ChassisNumber", MatchType.[Partial], txtNoChasis.Text))
        ...
        If txtSubject.Text <> String.Empty Then
            If txtSubject.Text.Split(";").Length > 1 Then
                ...
                For i As Integer = 0 To txtSubject.Text.Split(";").Length - 1
                    If i = 0 Then
                        criterias.opAnd(New Criteria(GetType(PQRHeader), "Subject", MatchType.[Partial], txtSubject.Text.Split(";")(i)), "(", True)
                    ElseIf i = txtSubject.Text.Split(";").Length - 1 Then
                        criterias.opAnd(New Criteria(GetType(PQRHeader), "Subject", MatchType.[Partial], txtSubject.Text.Split(";")(i)), ")", False)
                    Else
                        criterias.opAnd(New Criteria(GetType(PQRHeader), "Subject", MatchType.[Partial], txtSubject.Text.Split(";")(i)))
                    End If
                Next
            Else
                criterias.opAnd(New Criteria(GetType(PQRHeader), "Subject", MatchType.[Partial], txtSubject.Text))
            End If
        End If

        If txtKodeKerusakan.Text <> String.Empty Then criterias.opAnd(New Criteria(GetType(PQRHeader), "ID", MatchType.InSet, "(" & GenerateDamageCodeCriteria(txtKodeKerusakan.Text.Trim) & ")"))

        'If txtProsesOleh.Text <> String.Empty Then criterias.opAnd(New Criteria(GetType(PQRHeader), "ConfirmBy", MatchType.Partial, txtProsesOleh.Text))
            ...
            If txtProsesOleh.Text.Trim.Split("-".ToCharArray(), 2).Length > 1 Then
                ...
                If sOrg.Trim <> String.Empty Then
                    ...
                    ElseIf ListOrganization.Count > 1 Then
                        ...
                        Next
                    Else
                        criterias.opAnd(New Criteria(GetType(PQRHeader), "ConfirmBy", MatchType.[Partial], txtProsesOleh.Text.Trim.Split("-".ToCharArray(), 2)(1)))
                    End If
                Else
                    criterias.opAnd(New Criteria(GetType(PQRHeader), "ConfirmBy", MatchType.[Partial], txtProsesOleh.Text.Trim.Split("-".ToCharArray(), 2)(1)))
                End If
            Else
                criterias.opAnd(New Criteria(GetType(PQRHeader), "ConfirmBy", MatchType.[Partial], txtProsesOleh.Text))
            End If
        End If
        ...
        If IsForDownloadPurpose Then
            ...
            _arrPQRHeader = oPQRHeaderFacade.RetrieveByCriteria(criterias, CType(ViewState("CurrentSortColumn"), String), CType(ViewState("CurrentSortDirect"), Sort.SortDirection))
            sessHelper.SetSession("PQRDafterKondisi", _arrPQRHeader)
            Exit Sub
        Else
            _arrPQRHeader = oPQRHeaderFacade.RetrieveByCriteria(criterias, currentPageIndex + 1, dgListPQR.PageSize, _
                          total, CType(ViewState("CurrentSortColumn"), String), _
    ...
    Private Function GenerateDamageCodeCriteria(ByVal sCriteria As String) As String
        ...
        Dim strSQL As String = "Select PQRHeaderID from PQRDamageCode a join DeskripsiKodePosisi b on(a.DeskripsiKodePosisiID=b.ID)  where a.RowStatus=" & CType(DBRowStatus.Active, Short).ToString & " "
        ...
        If sCriteria.Trim.Split(";").Length > 1 Then
            ...
            For i As Integer = 0 To sCriteria.Trim.Split(";").Length - 1
                If i = 0 Then
                    ...
                    'lCriterias.opAnd(New Criteria(GetType(PQRDamageCode), "DeskripsiKodePosisi.KodePosition", MatchType.Partial, sCriteria.Trim.Split(";")(i)), "(", True)
                ElseIf i = sCriteria.Trim.Split(";").Length - 1 Then
                    strSQL &= " or KodePosition like '%" & sCriteria.Trim.Split(";")(i) & "%') "
                    'lCriterias.opOr(New Criteria(GetType(PQRDamageCode), "DeskripsiKodePosisi.KodePosition", MatchType.Partial, sCriteria.Trim.Split(";")(i)), ")", False)
                Else
                    strSQL &= " or KodePosition like '%" & sCriteria.Trim.Split(";")(i) & "%' "
                    'lCriterias.opOr(New Criteria(GetType(PQRDamageCode), "DeskripsiKodePosisi.KodePosition", MatchType.Partial, sCriteria.Trim.Split(";")(i)))
                End If
            Next

        Else
            strSQL &= " and KodePosition like '%" & sCriteria & "%' "
            'lCriterias.opAnd(New Criteria(GetType(PQRDamageCode), "DeskripsiKodePosisi.KodePosition", MatchType.Partial, sCriteria))
        End If
    ...
    Private Sub dgListPQR_ItemDataBound(ByVal sender As System.Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dgListPQR.ItemDataBound
        ...
        If e.Item.ItemIndex >= 0 Then
            ...
            TotalBobot = TotalBobot + objPQRHeader.Bobot

            Dim d As New DateTime(1753, 1, 1)
            Dim ts As TimeSpan
            ts = objPQRHeader.IntervalProcess.Subtract(d)

            Dim lblConfirmBy As Label = CType(e.Item.FindControl("lblConfirmBy"), Label)
            If objPQRHeader.ConfirmBy = String.Empty Then
                lblConfirmBy.Text = ""
            Else
                lblConfirmBy.Text = CommonFunction.FormatSavedUser(objPQRHeader.ConfirmBy, User)
            End If

            Dim lblTglRilis As Label = CType(e.Item.FindControl("lblTglRilis"), Label)
            If objPQRHeader.RealeseTime < New DateTime(1970, 1, 1) Then
                lblTglRilis.Text = ""
            Else
                lblTglRilis.Text = objPQRHeader.RealeseTime.ToString("dd/MM/yyyy")
            End If

            Dim lblTglValidasi As Label = CType(e.Item.FindControl("lblTglValidasi"), Label)
            If objPQRHeader.ValidationTime < New DateTime(1970, 1, 1) Then
                lblTglValidasi.Text = ""
            Else
                lblTglValidasi.Text = objPQRHeader.ValidationTime.ToString("dd/MM/yyyy")
            End If
            ...
            lblInterval.Text = objPQRHeader.Interval.Days.ToString & " Hari " & objPQRHeader.Interval.Hours.ToString & " Jam " & objPQRHeader.Interval.Minutes.ToString & " Menit"

            If objPQRHeader.Interval.Days > 3 And objPQRHeader.Interval.Days <= 7 And ((objPQRHeader.RowStatus = CType(EnumPQR.PQRStatus.Proses, Short)) Or (objPQRHeader.RowStatus = CType(EnumPQR.PQRStatus.Validasi, Short)) Or (objPQRHeader.RowStatus = CType(EnumPQR.PQRStatus.Rilis, Short))) Then
                e.Item.BackColor = Color.Yellow
            End If
            If objPQRHeader.Interval.Days > 7 And ((objPQRHeader.RowStatus = CType(EnumPQR.PQRStatus.Proses, Short)) Or (objPQRHeader.RowStatus = CType(EnumPQR.PQRStatus.Validasi, Short)) Or (objPQRHeader.RowStatus = CType(EnumPQR.PQRStatus.Rilis, Short))) Then
                e.Item.BackColor = Color.Red
            End If

            TotalInterval = TotalInterval + objPQRHeader.Interval.Ticks
        End If
    ...
    Private Sub btnDownload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDownload.Click
        ...
        _arrPQRHeader = sessHelper.GetSession("PQRDafterKondisi")

        For Each di As DataGridItem In dgListPQR.Items
            Dim chk As CheckBox = CType(di.FindControl("chkSelection"), CheckBox)
            If chk.Checked Then
                arlPQRToDownload.Add(CType(_arrPQRHeader(di.ItemIndex), PQRHeader))
                nChecked += 1
            End If
        Next
        If nChecked = 0 Then
        ...
        End If
        Response.Redirect("./FrmDownloadPQRListKondisi.aspx")
    End Sub
    ...
    Private Sub dgListPQR_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgListPQR.ItemCommand
        Select Case e.CommandName
            Case "ViewPQR"
                ...
                SetSessionCriteria()
                Server.Transfer("~/PQR/FrmPQRHeader.aspx?Mode=View&Src=PQRListKondisi")
        End Select
