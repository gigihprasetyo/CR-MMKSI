...
Public Class FrmPQRPrintPreview
...
#Region " Web Form Designer Generated Code "
  ...
  Private Sub Page_Init(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Init
    ...
    oPQRHeader = oPQRHeaderFacade.Retrieve(CInt(Request.QueryString("PQRID")))
    sessHelper.SetSession("PrintPQR", oPQRHeader)
    'oPQRHeader = CType(sessHelper.GetSession("PQR"), PQRHeader)

    RenderProfilePanel(oPQRHeader, New ProfileGroupFacade(User).Retrieve("pqr_prf"), EnumProfileType.ProfileType.PQR, pnlProfile1, True)
    RenderProfilePanel(oPQRHeader, New ProfileGroupFacade(User).Retrieve("pqr_prf_2"), EnumProfileType.ProfileType.PQR, pnlProfile2, True)
  End Sub
...
#End Region
  ...
  Private Sub LoadForm()
    oPQRHeader = CType(sessHelper.GetSession("PrintPQR"), PQRHeader)

    lbModel.Text = oPQRHeader.ChassisMaster.Category.Description

    If oPQRHeader.DocumentDate < New Date(1900, 1, 1) Then
      lblTglPembuatanVal.Text = ""
    Else
      lblTglPembuatanVal.Text = oPQRHeader.DocumentDate.ToString("dd/MM/yyyy")
    End If

    If oPQRHeader.ChassisMaster.DODate < New Date(1900, 1, 1) Then
      lblTglDeliveryVal.Text = ""
    Else
      lblTglDeliveryVal.Text = oPQRHeader.ChassisMaster.DODate.ToString("dd/MM/yyyy")
    End If

    If oPQRHeader.PQRDate < New Date(1900, 1, 1) Then
      lblTglKerusakanVal.Text = ""
    Else
      lblTglKerusakanVal.Text = oPQRHeader.PQRDate.ToString("dd/MM/yyyy")
    End If

    If oPQRHeader.ChassisMaster.EndCustomer Is Nothing OrElse oPQRHeader.ChassisMaster.EndCustomer.OpenFakturDate < New Date(1900, 1, 1) Then
      lblTglFakturVal.Text = ""
    Else
      lblTglFakturVal.Text = oPQRHeader.ChassisMaster.EndCustomer.OpenFakturDate.ToString("dd/MM/yyyy")
    End If

    If oPQRHeader.ChassisMaster.ProductionYear <= 0 Then
      lblThnProduksiVal.Text = ""
    Else
      lblThnProduksiVal.Text = oPQRHeader.ChassisMaster.ProductionYear.ToString()
    End If


    lblDealerVal.Text = oPQRHeader.Dealer.DealerCode & " - " & oPQRHeader.Dealer.SearchTerm1
    lblOdometerVal.Text = oPQRHeader.OdoMeter.ToString("#,##0")
    lblPQRNoVal.Text = oPQRHeader.PQRNo
    lblRefPQRNoVal.Text = oPQRHeader.RefPQRNo

    lblNoChasisVal.Text = oPQRHeader.ChassisMaster.ChassisNumber
    lblNoMesinVal.Text = oPQRHeader.ChassisMaster.EngineNumber
    lblTypeColorVal.Text = oPQRHeader.ChassisMaster.VechileColor.MaterialNumber & " - " & oPQRHeader.ChassisMaster.VechileColor.MaterialDescription
    If oPQRHeader.ChassisMaster.EndCustomer Is Nothing Then
      lblNamaVal.Text = ""
        Else
            If oPQRHeader.ChassisMaster.EndCustomer.Customer Is Nothing Then
                lblNamaVal.Text = ""
            Else
                lblNamaVal.Text = oPQRHeader.ChassisMaster.EndCustomer.Customer.Name1 & " - " & oPQRHeader.ChassisMaster.EndCustomer.Customer.Alamat
            End If
        End If
        ...
        If oPQRHeader.PQRDamageCodes.Count > 0 Then
            ...
            For Each item As PQRDamageCode In oPQRHeader.PQRDamageCodes
        ...
        If oPQRHeader.PQRPartsCodes.Count > 0 Then
            ...
            For Each item As PQRPartsCode In oPQRHeader.PQRPartsCodes
                sPart += item.SparePartMaster.PartNumber & " - " & item.SparePartMaster.PartName & "<br>"
            Next
            lblPartVal.Text = sPart
        End If

        lblKecepatanVal.Text = oPQRHeader.Velocity.ToString()
        lblSubjectVal.Text = oPQRHeader.Subject
        lblGejalaVal.Text = oPQRHeader.Symptomps.Replace(Chr(13) & Chr(10), "<br>")
        lblPenyebabVal.Text = oPQRHeader.Causes.Replace(Chr(13) & Chr(10), "<br>")
        lblHasilVal.Text = oPQRHeader.Results.Replace(Chr(13) & Chr(10), "<br>")
        lblCatatanVal.Text = oPQRHeader.Notes.Replace(Chr(13) & Chr(10), "<br>")

        If GetAttachmentList(oPQRHeader.PQRAttachments, EnumPQR.AttachmentLocation.Top).Count > 0 Then
            lblLampiranAtasVal.Text = "Ada"
        Else
            lblLampiranAtasVal.Text = "Tidak Ada"
        End If

        If oPQRHeader.ReleaseBy.Trim = String.Empty Then
            lblUserRilis.Text = ""
        Else
            lblUserRilis.Text = CommonFunction.FormatSavedUser(oPQRHeader.ReleaseBy, User)
        End If

        If oPQRHeader.RealeseTime < New DateTime(1900, 1, 1) Then
            lblTglRilis.Text = ""
        Else
            lblTglRilis.Text = oPQRHeader.RealeseTime.ToString("ddMMyyyy")
        End If

        lblSolution.Text = oPQRHeader.Solutions.Replace(Chr(13) & Chr(10), "<br>")

        If GetAttachmentList(oPQRHeader.PQRAttachments, EnumPQR.AttachmentLocation.Bottom).Count > 0 Then
            lblLampiranBawahVal.Text = "Ada"
        Else
            lblLampiranBawahVal.Text = "Tidak Ada"
        End If
        lblCodeA.Text = oPQRHeader.CodeA
        lblCodeB.Text = oPQRHeader.CodeB
        lblCodeC.Text = oPQRHeader.CodeC
        If oPQRHeader.PQRAdditionalInfos.Count > 0 Then ' generate additional info
            ...
            With divKomunikasi.Controls
                ...
                Dim x As Integer = 1
                For Each item As PQRAdditionalInfo In oPQRHeader.PQRAdditionalInfos
  ...
  Private Sub btnBatal_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBatal.Click
    ...
    '    Server.Transfer("~/PQR/FrmPQRList.aspx")
    'Else
    '    Server.Transfer("~/PQR/FrmPQRHeader.aspx?Mode=" & Request.QueryString("Mode") & "&Src=" & Request.QueryString("Src"))
    'End If

        Server.Transfer("~/PQR/FrmPQRHeader.aspx?Mode=" & Request.QueryString("Mode") & "&Src=" & Request.QueryString("Src") & "&PQRID=" & Request.QueryString("PQRID").ToString())
