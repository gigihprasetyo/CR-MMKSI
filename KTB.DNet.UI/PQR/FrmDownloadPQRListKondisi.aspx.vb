...
Public Class FrmDownloadPQRListKondisi
...
#End Region
    ...
    Private Sub WriteTraineeData(ByVal sw As StreamWriter, ByVal data As ArrayList)
        ...
        If Not IsNothing(data) Then
            ...
            For Each oPQRHeader As PQRHeader In data
                Try
                    itemLine.Remove(0, itemLine.Length)
                    'New Format
                    ...
                    itemLine.Append(oPQRHeader.Symptomps & tab) 'itemLine.Append("Gejala" & tab)
                    itemLine.Append(oPQRHeader.Causes & tab)  'itemLine.Append("Penyebab" & tab)
                    itemLine.Append(oPQRHeader.Results & tab) 'itemLine.Append("Perbaikan" & tab)
                    itemLine.Append(oPQRHeader.Notes & tab) 'itemLine.Append("Catatan" & tab)
    ...
    Private Sub WriteTraineeDataOld(ByVal sw As StreamWriter, ByVal data As ArrayList)
        ...
        If Not IsNothing(data) Then
            ...
            For Each item As PQRHeader In data
                ...
                For Each s As String In sProfileConfig.Split(";")
                    If item.PQRProfiles.Count > 0 Then

                        Dim isFound As Boolean = False
                        For Each _pqrprofile As PQRProfile In item.PQRProfiles
                            If _pqrprofile.ProfileHeader.Code = s Then
                                isFound = True
                                If _pqrprofile.ProfileValue = String.Empty OrElse _pqrprofile.ProfileValue Is Nothing Then
                                    itemLine.Append(" " & tab)
                                Else
                                    itemLine.Append(_pqrprofile.ProfileValue & tab)
                                End If
                                Exit For
                            End If
                        Next
                ...
                If item.PQRPartsCodes.Count > 0 Then
                    For Each Parts As PQRPartsCode In item.PQRPartsCodes
                        sw.WriteLine(header & Replace("K", "K", "") & tab & "P" & tab & Replace("Q", "Q", "") & tab & Parts.SparePartMaster.PartNumber & tab & " " & tab & " " & tab & " " & tab)
                    Next
                End If

                If item.PQRDamageCodes.Count > 0 Then
                    For Each DamageCode As PQRDamageCode In item.PQRDamageCodes
                ...
                If item.PQRQRSs.Count > 0 Then
                    For Each QRS As PQRQRS In item.PQRQRSs
                        _tglfaktur = String.Empty
                        _tglkerusakan = String.Empty
                        _dodate = String.Empty

                        If QRS.ChassisMaster.EndCustomer Is Nothing Then
                            _tglfaktur = ""
                        Else
                            If QRS.ChassisMaster.EndCustomer.OpenFakturDate < New Date(1900, 1, 1) Then
                                _tglfaktur = ""
                            Else
                                _tglfaktur = QRS.ChassisMaster.EndCustomer.OpenFakturDate.ToString("dd/MM/yyyy")
                            End If
                        End If
                        ...
                        _dodate = String.Empty
                        If QRS.ChassisMaster.DODate < New Date(1900, 1, 1) Then
                            _dodate = ""
                        Else
                            _dodate = QRS.TglKerusakan.ToString("dd/MM/yyyy")
                        End If

                        sw.WriteLine(header & Replace("K", "K", "") & tab & Replace("P", "P", "") & tab & "Q" & tab & QRS.ChassisMaster.ChassisNumber & tab & _tglkerusakan & tab & QRS.Odometer.ToString("#,##0") & tab & QRS.Note & tab & _dodate & tab & _tglfaktur & tab & QRS.ChassisMaster.EngineNumber & tab)
                    Next
                End If

                If item.PQRDamageCodes.Count = 0 And item.PQRPartsCodes.Count = 0 And item.PQRQRSs.Count = 0 Then
    ...
    Private Function GetStatus(ByVal obj As PQRHeader) As String
        ...
        Return result
    End Function
    Private Function GetIntervalTime(ByVal obj As PQRHeader) As String
        Dim d As New DateTime(1753, 1, 1)
        Dim ts As TimeSpan
        Dim result As String
