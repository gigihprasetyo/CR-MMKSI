...
#End Region
    ...
    Private Sub RetrieveMaster()
        ...
        Dim _arrCat As ArrayList = New PQRHeaderFacade(User).RetrieveListCategory()
        ...
        Dim _arrTipe As ArrayList = New PQRHeaderFacade(User).RetrieveListType()
    ...
    Private Sub btnCari_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCari.Click
        ...
        With pnlResult.Controls
            If Not _arrResult.Count = 0 Then
                ...
                For Each _item As PQRHeader In _arrResult
                    ...
                    lblKategori.Text = _item.ChassisMaster.Category.Description
                    .Add(lblKategori)
                    ...
                    lblTipe.Text = _item.ChassisMaster.VechileColor.MaterialDescription
                    .Add(lblTipe)
                    ...
                    Dim criDamage As New CriteriaComposite(New Criteria(GetType(PQRDamageCode), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
                    criDamage.opAnd(New Criteria(GetType(PQRDamageCode), "PQRHeader.ID", MatchType.Exact, _item.ID))
                    Dim _resultDamage2 As ArrayList = New PQRDamageCodeFacade(User).Retrieve(criDamage)
                    Dim lblDamageCode As New LinkButton
                    If _resultDamage2.Count > 0 Then
                        For Each _itemDamage As PQRDamageCode In _resultDamage2
                    ...
                    Dim criParts As New CriteriaComposite(New Criteria(GetType(PQRPartsCode), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
                    criParts.opAnd(New Criteria(GetType(PQRPartsCode), "PQRHeader.ID", MatchType.Exact, _item.ID))
                    Dim _resultParts2 As ArrayList = New PQRPartsCodeFacade(User).Retrieve(criParts)
                    Dim lblPartCode As New Label
                    If _resultParts2.Count > 0 Then
                        For Each _itemParts As PQRPartsCode In _resultParts2
        ...
        End With
    End Sub
    Private Function CollectionPQRHeader() As ArrayList
        Dim criterias As New CriteriaComposite(New Criteria(GetType(PQRHeader), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
        ...
        If Not ddlKategori.SelectedValue = "null" Then
            criterias.opAnd(New Criteria(GetType(PQRHeader), "ChassisMaster.Category.CategoryCode", MatchType.Exact, ddlKategori.SelectedValue))
        End If
        If Not ddlTipe.SelectedValue = "null" Then
            criterias.opAnd(New Criteria(GetType(PQRHeader), "ChassisMaster.VechileColor.ColorCode", MatchType.Exact, ddlTipe.SelectedValue))
        End If

        Dim tempSbj() As String = txtSubject.Text.Trim.Split(" ")
        If tempSbj.Length > 1 Then
            For Each _item As String In tempSbj
                criterias.opAnd(New Criteria(GetType(PQRHeader), "Subject", MatchType.[Partial], _item))
            Next
        Else
            criterias.opAnd(New Criteria(GetType(PQRHeader), "Subject", MatchType.[Partial], txtSubject.Text.Trim))
        End If

        Dim _result As ArrayList = New PQRHeaderFacade(User).RetrieveByCriteria(criterias)
        Return _result
    End Function
    Private Function CollectionDamage() As ArrayList
        ...
        If Not txtKodeKerusakan.Text.Trim() = "" Then
            ...
            For Each _item As DeskripsiKodePosisi In _resultDamage
                Dim cri As New CriteriaComposite(New Criteria(GetType(PQRDamageCode), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
                cri.opAnd(New Criteria(GetType(PQRDamageCode), "DeskripsiKodePosisi.ID", MatchType.Exact, _item.ID))
                Dim _arrTemp As ArrayList = New PQRDamageCodeFacade(User).Retrieve(cri)
                For Each _itemPQR As PQRDamageCode In _arrTemp
    ...
    Private Function CollectionParts() As ArrayList
        ...
        If Not txtKodePart.Text.Trim() = "" Then
            ...
            For Each _item As SparePartMaster In _resultParts
                Dim cri As New CriteriaComposite(New Criteria(GetType(PQRPartsCode), "RowStatus", MatchType.Exact, CType(DBRowStatus.Active, Short)))
                cri.opAnd(New Criteria(GetType(PQRPartsCode), "SparePartMaster.ID", MatchType.Exact, _item.ID))
                Dim _arrTemp As ArrayList = New PQRPartsCodeFacade(User).Retrieve(cri)
                For Each _itemPQR As PQRPartsCode In _arrTemp
    ...
    Private Function ResultCollection() As ArrayList
        Dim _resultPQRHeader As ArrayList = CollectionPQRHeader()
        Dim _resultDamage As ArrayList = CollectionDamage()
        Dim _resultParts As ArrayList = CollectionParts()
        Dim _result As New ArrayList

        For Each itemheader As PQRHeader In _resultPQRHeader
            _result.Add(itemheader)
        Next

        For Each _itemDamage As PQRDamageCode In _resultDamage
            _result.Add(_itemDamage.PQRHeader)
        Next

        For Each _itemParts As PQRPartsCode In _resultParts
            _result.Add(_itemParts.PQRHeader)
        Next
        ...
        For i = 0 To _result.Count - 1
            ...
            For j = 0 To _result.Count - 1
                ...
                Dim obj1 As PQRHeader = CType(_result(i), PQRHeader)
                Dim obj2 As PQRHeader = CType(_result(j), PQRHeader)
                If i <> j Then
            ...
            Next
            If bcheck Then
                _resultFinal.Add(CType(_result(i), PQRHeader))
            End If
            i = i + 1
        Next
        Return _resultFinal
    ...
    Private Sub btnBack_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBack.Click
        Response.Redirect("FrmPQRAdditionalInfo.aspx?PQRID=25")
    End Sub
End Class
