...
Public Class FrmSalesmanPartPerformanceList
...
#Region "Privilege"
    Private Sub CheckPrivilege()
        If Not SecurityProvider.Authorize(context.User, SR.Lihat_daftar_salesman_performance_privilege) Then
            Server.Transfer("../FrmAccessDenied.aspx?modulName=Part Employee - Daftar Salesman Performance")
        End If
    End Sub
    Dim Priv_All_Download As Boolean = SecurityProvider.Authorize(context.User, SR.Download_daftar_salesman_performance_privilege)
#End Region
...
#Region "Event handler"
    ...
    Private Sub dgSalesmanPerformance_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dgSalesmanPerformance.ItemDataBound
        If Not e.Item.DataItem Is Nothing Then
            ...
            CType(e.Item.FindControl("lblNo"), Label).Text = e.Item.ItemIndex + 1 + (dgSalesmanPerformance.CurrentPageIndex * dgSalesmanPerformance.PageSize)
            lblSearchTerm2.Text = objSalesmanPartPerformance.SalesmanHeader.Dealer.SearchTerm2
            lblKodeDealer.Text = objSalesmanPartPerformance.SalesmanHeader.Dealer.DealerCode
            lblMonth.Text = enumMonthGet.GetName(objSalesmanPartPerformance.Month) 'CType(objSalesmanPartTarget.Month, enumMonth.Month).ToString.Replace("_", " ")
    ...
    Private Sub dgSalesmanPerformance_PageIndexChanged(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridPageChangedEventArgs) Handles dgSalesmanPerformance.PageIndexChanged
        ...
        BindSalesmanPerformance(dgSalesmanPerformance.CurrentPageIndex)
    End Sub

    Private Sub dgSalesmanPerformance_SortCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridSortCommandEventArgs) Handles dgSalesmanPerformance.SortCommand
        ...
        Else
            ...
            ViewState("CurrentSortDirect") = Sort.SortDirection.ASC
        End If
        dgSalesmanPerformance.SelectedIndex = -1
        BindSalesmanPerformance(dgSalesmanPerformance.CurrentPageIndex)
    End Sub

    Private Sub btnCari_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnCari.Click
        BindSalesmanPerformance()
        arlPerformance = CType(sessHelper.GetSession("PerformanceList"), ArrayList)
        If arlPerformance.Count <= 0 Then
...
#Region "Custom"
    ...
    Private Sub BindControlsAttribute()
        ...
        sessHelper.SetSession("strFileNm", "SalesmanPartPerformance")
        sessHelper.SetSession("strFileNmHeader", "Salesman Part Performance")
    End Sub
    ...
    Private Sub BindSalesmanPerformance()
        ViewState("CurrentSortColumn") = "SalesmanHeader.ID"
        ViewState("CurrentSortDirect") = Sort.SortDirection.ASC
        If Not IsNothing(sessHelper.GetSession("idxPage")) Then
            BindSalesmanPerformance(sessHelper.GetSession("idxPage"))
        Else
            BindSalesmanPerformance(0)
        End If
    End Sub

    Private Sub BindSalesmanPerformance(ByVal idxPage As Integer)
        ...
        If (txtDealerCode.Text.Trim <> String.Empty) Then
            criterias.opAnd(New Criteria(GetType(SalesmanPartPerformance), "SalesmanHeader.Dealer.DealerCode", MatchType.InSet, "('" + Replace(txtDealerCode.Text, ";", "','") + "')"))
        End If
        If ddlPeriodMonth1.SelectedIndex <> 0 Then
            If ddlPeriodYear1.SelectedIndex <> 0 Then
                criterias.opAnd(New Criteria(GetType(SalesmanPartPerformance), "Period", MatchType.GreaterOrEqual, New Date(CType(ddlPeriodYear1.SelectedValue, Integer), CType(ddlPeriodMonth1.SelectedValue, Integer), 1, 0, 0, 0)))
            Else
        ...
        If ddlPeriodMonth2.SelectedIndex <> 0 Then
            If ddlPeriodYear2.SelectedIndex <> 0 Then
                criterias.opAnd(New Criteria(GetType(SalesmanPartPerformance), "Period", MatchType.LesserOrEqual, New Date(CType(ddlPeriodYear2.SelectedValue, Integer), CType(ddlPeriodMonth2.SelectedValue, Integer), 1, 23, 59, 59)))
            Else
        ...
        If txtSalesmanCode.Text.Trim <> String.Empty Then
            criterias.opAnd(New Criteria(GetType(SalesmanPartPerformance), "SalesmanHeader.SalesmanCode", MatchType.Exact, txtSalesmanCode.Text.Trim))
        End If
        ...
        sessHelper.SetSession("idxPage", dgSalesmanPerformance.CurrentPageIndex)
        sessHelper.SetSession("PerformanceList", arlPerformance)
    ...
    Private Sub SetDownload()
        ...
        If dgSalesmanPerformance.Items.Count < 1 Then
        ...
        arrData = New SalesmanPartPerformanceFacade(User).RetrieveByCriteria(crits)
    ...
    Private Sub WriteListData(ByVal sw As StreamWriter, ByVal data As ArrayList)
        ...
        If Not IsNothing(data) Then
            ...
            itemLine.Append("Harga Cost Penjualan" & tab)
            itemLine.Append("Harga Retail Penjualan" & tab)
            itemLine.Append("Profit" & tab)
            itemLine.Append("Profit (%)" & tab)
            sw.WriteLine(itemLine.ToString())

            Dim i As Integer = 1
            For Each item As SalesmanPartPerformance In data
                itemLine.Remove(0, itemLine.Length)
                itemLine.Append(i.ToString & tab)
                itemLine.Append(item.SalesmanHeader.Dealer.SearchTerm2 & tab)
                ...
                itemLine.Append(item.SalesmanHeader.Name.ToString & tab)
                If item.SalesmanHeader.SalesmanAdditionalInfo.Count > 0 Then
                    Dim salesmanInfo As SalesmanAdditionalInfo = item.SalesmanHeader.SalesmanAdditionalInfo(0)
                    itemLine.Append(salesmanInfo.SalesmanCategoryLevel.SalesmanCategoryLevel.PositionName & tab)
                    itemLine.Append(salesmanInfo.SalesmanCategoryLevel.PositionName & tab)
                    Dim enumLevel As EnumSalesmanPartLevel.Level = salesmanInfo.SalesmanLevel
                    itemLine.Append(enumLevel.ToString & tab)
                Else
                    itemLine.Append(String.Empty & tab)
                    itemLine.Append(String.Empty & tab)
                    itemLine.Append(String.Empty & tab)
                End If
                itemLine.Append(item.SalesmanHeader.SalesmanArea.AreaDesc & tab)
                itemLine.Append(CType(item.SalesmanHeader.Status, EnumSalesmanStatus.SalesmanStatus).ToString.Replace("_", " ") & tab)
                itemLine.Append(item.Year & tab)
                itemLine.Append(item.Month & tab)
                itemLine.Append(FormatNumber(item.HargaPokok, 2, TriState.UseDefault, TriState.UseDefault, TriState.True) & tab)
                itemLine.Append(FormatNumber(item.HargaJual, 2, TriState.UseDefault, TriState.UseDefault, TriState.True) & tab)
                itemLine.Append(FormatNumber(item.Profit, 2, TriState.UseDefault, TriState.UseDefault, TriState.True) & tab)
                itemLine.Append(FormatNumber(item.Percentage, 2, TriState.UseDefault, TriState.UseDefault, TriState.True) & tab)
                sw.WriteLine(itemLine.ToString())
                i = i + 1
            Next
        End If
