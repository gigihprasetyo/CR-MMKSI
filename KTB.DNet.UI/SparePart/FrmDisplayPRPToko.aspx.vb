...
Namespace KTB.DNet.UI.SparePart
    ...
    Public Class FrmDisplayPRPToko
...
#Region "Event Method"
        ...
        Private Sub dtgExcel_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles dtgExcel.ItemDataBound
            Const CellJan As Integer = 4
            Const CellFeb As Integer = 5
            Const CellMar As Integer = 6
            Const CellApr As Integer = 7
            Const CellMay As Integer = 8
            Const CellJun As Integer = 9
            Const CellJul As Integer = 10
            Const CellAug As Integer = 11
            Const CellSep As Integer = 12
            Const CellOct As Integer = 13
            Const CellNov As Integer = 14
            Const CellDec As Integer = 15
            Const CellTot As Integer = 16
...
#Region "General Method"
        Private Sub initiatePage()
            ...
            End If
            sHPrp.SetSession("DisplayPRP", "PRPToko")
        ...
        Private Sub bindDataGrid()
            ...
            If IsNothing(dtTable) Then
                ...
                Try
                    ...
                    Dim Data As ArrayList = excelParser.ParseExcelNoTransaction(filePath, "[PERTOKO$]", User.Identity.Name)
...
#Region "Convert to Data Table"
        Private Function ConvertToDataTable(ByVal dtArray As ArrayList) As DataTable
            ...
            For rowIndex As Integer = 0 To dtArray.Count - 1
                Dim objPRP As PRPExcelPerToko = CType(dtArray(rowIndex), PRPExcelPerToko)
        ...
        Private Function toRow(ByVal dtTable As DataTable, ByVal data As PRPExcelPerToko) As DataRow
        ...
        Private Function GenerateColumn() As DataColumn()
            Dim dtColColl(15) As DataColumn

            dtColColl(0) = New DataColumn("PSCode", GetType(String))
            dtColColl(1) = New DataColumn("PartShopName", GetType(String))
            dtColColl(2) = New DataColumn("Kota", GetType(String))
            dtColColl(3) = New DataColumn("Jan", GetType(Int32))
            dtColColl(4) = New DataColumn("Feb", GetType(Int32))
            dtColColl(5) = New DataColumn("Mar", GetType(Int32))
            dtColColl(6) = New DataColumn("Apr", GetType(Int32))
            dtColColl(7) = New DataColumn("May", GetType(Int32))
            dtColColl(8) = New DataColumn("Jun", GetType(Int32))
            dtColColl(9) = New DataColumn("Jul", GetType(Int32))
            dtColColl(10) = New DataColumn("Aug", GetType(Int32))
            dtColColl(11) = New DataColumn("Sep", GetType(Int32))
            dtColColl(12) = New DataColumn("Oct", GetType(Int32))
            dtColColl(13) = New DataColumn("Nov", GetType(Int32))
            dtColColl(14) = New DataColumn("Dec", GetType(Int32))
            dtColColl(15) = New DataColumn("Total", GetType(Int32))
