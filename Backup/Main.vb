...
Module Main
    ...
    Private Sub Announce(ByVal IsSuccess As Boolean, ByVal strMessage As String)
        Dim SMSTos As String = ConfigurationSettings.AppSettings("SMSRecipient")
        Dim EmailTos As String = ConfigurationSettings.AppSettings("EmailRecipient")
        Dim smtp As String = ConfigurationSettings.AppSettings("SMTP")
        ...
        If ConfigurationSettings.AppSettings("AlwaysSendSMS").Trim.ToLower = "true" Then IsAlwaysSendSMS = True

        For Each sEmail As String In EmailTos.Split(";")
            If sEmail.Trim <> "" Then
                Try
                    ObjEmail.sendMail(sEmail, "", emailFrom, "[WSM Checking]", System.Web.Mail.MailFormat.Html, strMessage)
                    ...
                    If sSmsNo.Trim <> "" Then
                        Try
                            If objSMS.IsSMSGatewayLive Then
                                objSMS.Sendto(sSmsNo, "[WSM Checking] " & strMessage)
                            Else
    ...
    Private Function RestartService() As Boolean
        Dim SVCName As String = ConfigurationSettings.AppSettings("ServiceName")
        Try
    ...
    Private Sub WriteLog(ByVal str As String)
        Dim FolderToSave As String = "C:\Services\"
        Dim FileName As String = "ServiceLog" & ".log"
        Dim oFI As FileInfo
