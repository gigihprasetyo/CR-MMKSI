...
<STYLE>
TD.cal { FONT-SIZE: 11px; COLOR: #000000; FONT-FAMILY: Arial,Helvetica,Sans-serif; BACKGROUND-COLOR: #d3d3d3 } 
SELECT.month { FONT-SIZE: 11px; WIDTH: 85px; COLOR: #000000; FONT-FAMILY: Arial,Helvetica,Sans-serif } 
INPUT.year { FONT-SIZE: 11px; WIDTH: 30px; COLOR: #000000; FONT-FAMILY: Arial,Helvetica,Sans-serif } 
TD.calDaysColor { FONT-SIZE: 11px; COLOR: #ffffff; FONT-FAMILY: Arial,Helvetica,Sans-serif; BACKGROUND-COLOR: #000000 } 
TD.calWeekend { FONT-SIZE: 11px; COLOR: #ffffff; FONT-FAMILY: Arial,Helvetica,Sans-serif; BACKGROUND-COLOR: #c0c0c0 } 
TD.calBgColor { FONT-SIZE: 11px; COLOR: #ffffff; FONT-FAMILY: Arial,Helvetica,Sans-serif; BACKGROUND-COLOR: #d3d3d3 } 
.calBorderColor { FONT-SIZE: 11px; COLOR: #ffffff; FONT-FAMILY: Arial,Helvetica,Sans-serif; BACKGROUND-COLOR: #a9a9a9 } 
TD.calHighlightColor { FONT-SIZE: 11px; COLOR: #ffffff; FONT-FAMILY: Arial,Helvetica,Sans-serif; BACKGROUND-COLOR: #ffffcc } 
A.cal { FONT-SIZE: 11px; COLOR: #000000; FONT-FAMILY: Arial,Helvetica,Sans-serif; TEXT-DECORATION: none } 
...
// script 
   ...
   var g_startDay = 0 // 0=sunday, 1=monday  
	...
	if (browser.dom || browser.ie4){  
		...
		document.writeln('#containerBackgroundLayer {');
		document.writeln('position : absolute;');  
		document.writeln('left : 100px;');  
		document.writeln('top : 100px;');  
		document.writeln('width : 124px;');;  
		...
		document.writeln('}');
		document.writeln('</style>')  
		document.write('<div id="containerBackgroundLayer"><iframe width="100%" height="100%"></iframe></div>');
	...
	function Calendar(){  
	  ...
	  this.longDaysOfWeek = new Array@LONG_DAY_OF_WEEK@;
	  this.daysOfWeek = new Array@SHORT_DAY_OF_WEEK@;  
	  this.months = new Array@LONG_MONTH_OF_YEAR@;  
	  ...
	  if (browser.ns4){  
	    ...
	    var tmpBackgroundLayer = new Layer(127);
	    tmpBackgroundLayer.x = 100;
	    tmpBackgroundLayer.y = 100;
	    tmpBackgroundLayer.bgColor = "#ffffff";
	  }  
	  if (browser.dom || browser.ie4){  
		var tmpLayer = browser.dom?document.getElementById('container'):document.all.container;  
		var tmpBackgroundLayer = browser.dom?document.getElementById('containerBackgroundLayer'):document.all.containerBackgroundLayer;
	  }  
	  this.containerLayer = tmpLayer; 
	  this.containerBackgroundLayer = tmpBackgroundLayer;
	  
	  this.containerLayer.style.zIndex=1000;
	  this.containerBackgroundLayer.style.zIndex=999;
	  
	  if (browser.ns4 && browser.platform=='Win32') {  
	    this.containerLayer.clip.height=134;  
	    this.containerLayer.clip.width=127;
	    this.containerBackgroundLayer.clip.height=134;
	    this.containerBackgroundLayer.clip.width=127;  
	  }  
	  ...
	  for (var j=1;j<=6;j++){ 
	     ...
	     for (var i=1;i<=7;i++){ 
		   ...
		   if ( (7*(j-1) + i)>=iFirstDOM+1  && iCount <= iDaysInMonth){ 
			 ...
			 var disabled = false;
			 for (dis=0; dis<this.disable.length; dis++) {
				if (this.longDaysOfWeek[(i-g_startDay+13)%7]==this.disable[dis]) {
					disabled = true;
					break;
				}
			 }
   ...
   // console.log(this.target)
	   ...
	   if (tmp.onblur)
	   {
	    ...
	    {	
			...
			this.containerBackgroundLayer.hidden=true;
		}
	    if (browser.dom || browser.ie4){ 	    
	      this.containerLayer.style.visibility='hidden'; 
	      this.containerBackgroundLayer.style.visibility='hidden';
	    } 
	} 
...
this.containerLayer.innerHTML = str; 
	...
	Calendar.prototype.show = function(event, target, bHasDropDown, dateFormat, pdisable, dateFrom, dateTo){ 
    //console.log(target);
	...
	this.disable = (pdisable) ? pdisable.split(/\,/) : new Array();
...
this.containerLayer.style.visibility='hidden'; 
	  ...
	  if (browser.ns4){ 
		this.containerBackgroundLayer.x = event.x + 10;
		this.containerBackgroundLayer.y = event.y - 5;
	  ...
	  if (browser.ie5 || browser.ie4){ 
        ...
        this.containerBackgroundLayer.style.left = x;
		if (event.y>0)
		{	
			this.containerLayer.style.top = y; 
			this.containerBackgroundLayer.style.top = y;
		}
	  } 
	  if (browser.ns6){ 
		this.containerBackgroundLayer.style.left = event.pageX + 10;
		this.containerBackgroundLayer.style.top = event.pageY - 5;
	  ...
	  // and then show it! 
	   ...
	   this.containerBackgroundLayer.hidden = false;
	   } 
	  if (browser.dom || browser.ie4){ 
	      this.containerLayer.style.visibility='visible'; 
	      this.containerBackgroundLayer.style.visibility='visible';
	  } 
	} 
...
// Finally licked extending  native date object; 
	...
	/*window.onerror = function(msg,url,line){ 
	  alert('******* an error has occurred ********') 
				objScriptCal.Append("	  alert('******* an error has occurred ********' + 
				objScriptCal.Append("	  '\n\nPlease check that' + 
